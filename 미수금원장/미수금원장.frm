...
' 참조된 Table   :
' 업  무  설  명 : 매출장부이력(매출세금계산서장부)조회 + 매출처수금처리
'+-------------------------------------------------------------------------------------------------------+
Option Explicit
...
ERROR_TABLE_SELECT:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "미수금 추가 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_INSERT:
    PB_adoCnnSQL.RollbackTrans
...
ERROR_TABLE_UPDATE:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "미수금 저장 실패"
    Unload Me
    Exit Sub
End Sub
...
Dim lngC    As Long
    With vsfg1              'Rows 1, Cols 13, RowHeightMax(Min) 300
         ...
         .ColWidth(1) = 1000   '매출처코드
         .ColWidth(2) = 2000   '매출처명
         .ColWidth(3) = 1000   '일자
         .ColWidth(4) = 3000   '적요
         ...
         .ColWidth(9) = 2000   '미수금액
         .ColWidth(10) = 2000  '입금금액
         .ColWidth(11) = 2000  '잔액
         .ColWidth(12) = 4000  '비고
         ...
         .TextMatrix(0, 1) = "매출처코드"  'H
         .TextMatrix(0, 2) = "매출처명"    'H
         .TextMatrix(0, 3) = "날짜"
         .TextMatrix(0, 4) = "적요"
         .TextMatrix(0, 5) = "구분"        'H
         .TextMatrix(0, 6) = "구분"
...
'+------------------------------------------------------------------+
'/// VsFlexGrid(vsfg1) 채우기(미수금발생구분 : 1.전표, 2.계산서) ///
'+------------------------------------------------------------------+
...
Dim curMonOMny  As Currency  '해당월누계금액(출고)
Dim curMonIMny  As Currency  '해당월누계금액(입급)
Dim curMonTMny  As Currency  '해당월누계금액(잔액)
Dim curTotOMny  As Currency  '해당누계금액(출고)
Dim curTotIMny  As Currency  '해당누계금액(입금)
Dim curTotTMny  As Currency  '해당누계금액(잔액)
Dim curTotTOMny As Currency  '전체누계금액(출고)
Dim curTotTIMny As Currency  '전체누계금액(입금)
Dim curTotTTMny As Currency  '전체누계금액(잔액)
    ...
    End With
    strOrderBy = "ORDER BY T1.사업장코드, " & IIf(optPrtChk0.Value = True, "T1.매출처코드, T3.매출처명 ", "T3.매출처명, T1.매출처코드 ") & ", 일자 , 시간, 구분 "
    P_adoRec.CursorLocation = adUseClient
    strSQL = "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  & "T1.매출처코드 AS 매출처코드, T3.매출처명 AS 매출처명, " _
                  & "(T1.마감년월 + '00') AS 일자, '(년 이 월)' AS 적요, " _
                  & "0 AS 구분, (T1.미수금누계금액) AS 출고금액, " _
                  & "(T1.미수금입금누계금액) AS 입금금액, " _
             ...
             & "" & strWhere & " AND SUBSTRING(T1.마감년월,5,2) = '00' AND (T1.미수금누계금액 <> 0 OR T1.미수금입금누계금액 <> 0) " _
    ...
    If PB_regUserinfoU.UserMSGbn = "1" Then
       strSQL = strSQL & "UNION ALL " _
           & "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  & "T1.매출처코드 AS 매출처코드, T3.매출처명 AS 매출처명, " _
                  & "(T1.입출고일자) AS 일자, (T1.적요) AS 적요, " _
                  & "T1.입출고구분 AS 구분, (SUM(T1.출고수량 * T1.출고단가) * " & (PB_curVatRate + 1) & ") AS 출고금액, " _
                  & "0 AS 입금금액, " _
             ...
             & "LEFT JOIN 사업장 T2 ON T2.사업장코드 = T1.사업장코드 " _
             & "LEFT JOIN 매출처 T3 ON T3.사업장코드 = T1.사업장코드 AND T3.매출처코드 = T1.매출처코드 " _
             & "" & strWhere & " AND (T1.입출고구분 = 2 OR T1.입출고구분 = 8) " _
              & "AND (T1.사용구분 = 0) " _
              & "AND T1.입출고일자 BETWEEN '" & (Mid(DTOS(dtpF_Date.Value), 1, 6) + "01") & "' AND '" & DTOS(dtpT_Date.Value) & "' " _
            & "GROUP BY T1.사업장코드, T2.사업장명, T1.매출처코드, T3.매출처명, " _
                     & "T1.입출고일자, T1.입출고시간, T1.적요, T1.현금구분, " _
                     & "T1.입출고구분 "
    Else
       strSQL = strSQL & "UNION ALL " _
           & "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  ...
                  & "2 AS 구분, (SUM(T1.공급가액 + T1.세액)) AS 출고금액, " _
                  & "0 AS 입금금액, " _
    ...
    strSQL = strSQL & "UNION ALL " _
           & "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  & "T1.매출처코드 AS 매출처코드, T3.매출처명 AS 매출처명, " _
                  & "(T1.미수금입금일자) AS 일자, T1.적요 AS 적요, " _
                  & "0 AS 구분, 0 AS 출고금액, " _
                  & "ISNULL(SUM(T1.미수금입금금액), 0) As 입금금액, " _
                  & "결제방법 = CASE WHEN T1.결제방법 = 0 THEN '현금' WHEN T1.결제방법 = 1 THEN '수표' " _
                                  & "WHEN T1.결제방법 = 2 THEN '어음' ELSE '기타' END, " _
                  & "T1.만기일자, T1.어음번호, T1.미수금입금시간 AS 시간 " _
             & "FROM 미수금내역 T1 " _
             & "LEFT JOIN 사업장 T2 ON T2.사업장코드 = T1.사업장코드 " _
             & "LEFT JOIN 매출처 T3 ON T3.사업장코드 = T1.사업장코드 AND T3.매출처코드 = T1.매출처코드 " _
             & "" & strWhere & " AND T1.미수금입금일자 " _
                         & "BETWEEN '" & (Mid(DTOS(dtpF_Date.Value), 1, 6) + "01") & "' AND '" & DTOS(dtpT_Date.Value) & "' " _
            & "GROUP BY T1.사업장코드, T2.사업장명, T1.매출처코드, T3.매출처명, " _
                     & "T1.미수금입금일자, T1.미수금입금시간, T1.적요, T1.결제방법, T1.만기일자, T1.어음번호 "
    strSQL = strSQL _
           & "" & strOrderBy & " "
    ...
    Else
       With vsfg1
            ...
            .AddItem ""
            lngR = lngR + 1
            .TextMatrix(lngR, 3) = P_adoRec("매출처코드"): .TextMatrix(lngR, 4) = P_adoRec("매출처명")
            .Cell(flexcpBackColor, lngR, 0, lngR, .Cols - 1) = vbYellow
            Do Until P_adoRec.EOF
               lngR = lngR + 1
               If lngR <> 2 Then    '처음 레코드 아니면
                  If .TextMatrix(lngR - 1, 1) <> P_adoRec("매출처코드") Then '매출처코드가 다르면
                     If .TextMatrix(lngR - 1, 3) <> "" Then '일 집계면
                        '해당월누계
                         .AddItem ""
                         '.TextMatrix(lngR, 4) = "(" & Format(Mid(StrDate, 1, 6), "0000-00") & Space(1) & "월계)"
                         .TextMatrix(lngR, 4) = "(월    계)"
                         .TextMatrix(lngR, 9) = curMonOMny  '월누계금액(출고)
                         .TextMatrix(lngR, 10) = curMonIMny '월누계금액(입금)
                         curMonOMny = 0: curMonIMny = 0: curMonTMny = 0
                         lngR = lngR + 1
                     End If
                     '해당누계
                     .TextMatrix(lngR, 4) = "(누    계)"
                     .TextMatrix(lngR, 9) = curTotOMny      '매출처누계금액(출고)
                     .TextMatrix(lngR, 10) = curTotIMny     '매출처누계금액(입금)
                     curTotOMny = 0: curTotIMny = 0: curTotTMny = 0
                     .AddItem ""
                     lngR = lngR + 1
                     .AddItem ""
                     lngR = lngR + 1
               ...
               .TextMatrix(lngR, 1) = P_adoRec("매출처코드")
               .TextMatrix(lngR, 2) = P_adoRec("매출처명")
               '3. 일자
               ...
               If PB_regUserinfoU.UserMSGbn = "1" Then
                  If P_adoRec("구분") = 0 Then
                     .TextMatrix(lngR, 6) = IIf(Mid(P_adoRec("일자"), 7, 2) <> "00", "수금", "") + IIf(P_adoRec("결제방법") = "", "", "(" + P_adoRec("결제방법") + ")")
                  ElseIf _
                     P_adoRec("구분") = 2 Then
                     .TextMatrix(lngR, 6) = "매출" + IIf(P_adoRec("결제방법") = "", "", "(" + P_adoRec("결제방법") + ")")
                  ElseIf _
                     P_adoRec("구분") = 8 Then
                     .TextMatrix(lngR, 6) = "임의" + IIf(P_adoRec("결제방법") = "", "", "(" + P_adoRec("결제방법") + ")")
                  End If
               Else
                  If P_adoRec("구분") = 0 Then
                     .TextMatrix(lngR, 6) = IIf(Mid(P_adoRec("일자"), 7, 2) <> "00", "수금", "") + IIf(P_adoRec("결제방법") = "", "", "(" + P_adoRec("결제방법") + ")")
               ...
               End If
               '9. 출고금액
               .TextMatrix(lngR, 9) = P_adoRec("출고금액")
               '10. 입금액
               .TextMatrix(lngR, 10) = P_adoRec("입금금액")
               '11. 잔액
               .TextMatrix(lngR, 11) = curTotTMny + (P_adoRec("출고금액") - P_adoRec("입금금액"))
               '12. 적요
               If Mid(P_adoRec("일자"), 7, 2) = "00" Then
               Else
               ...
               If Mid(P_adoRec("일자"), 7, 2) <> 0 Then
                  curMonOMny = curMonOMny + P_adoRec("출고금액")
                  curMonIMny = curMonIMny + P_adoRec("입금금액")
                  curMonTMny = curMonTMny + (P_adoRec("출고금액") - P_adoRec("입금금액"))
               End If
               '해당누계금액
               curTotOMny = curTotOMny + P_adoRec("출고금액")
               curTotIMny = curTotIMny + P_adoRec("입금금액")
               curTotTMny = curTotTMny + (P_adoRec("출고금액") - P_adoRec("입금금액"))
               '전체누계금액
               curTotTOMny = curTotTOMny + P_adoRec("출고금액")
               curTotTIMny = curTotTIMny + P_adoRec("입금금액")
               curTotTTMny = curTotTTMny + (P_adoRec("출고금액") - P_adoRec("입금금액"))
               StrDate = P_adoRec("일자")
               'FindRow 사용을 위해
            ...
            If chkTotal.Value = 1 Then
               ...
               .TextMatrix(lngR, 9) = curTotTOMny        '전체누계금액(출고)
               .TextMatrix(lngR, 10) = curTotTIMny       '전체누계금액(입금)
...
ERROR_TABLE_SELECT:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "미수금 읽기 실패"
    Unload Me
    Exit Sub
End Sub
Private Sub Subvsfg1_FILL_1() 'Not Used
Dim strSQL      As String
Dim strJoin     As String
Dim strGroupBy  As String
Dim strHaving   As String
...
Dim curTotTTMny As Currency  '전체누계금액(잔액)
    ...
    With vsfg1
         ...
         strWhere = strWhere + IIf(Trim(strWhere) = "", "WHERE ", "AND ") _
                  & "T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' "
    End With
    strOrderBy = "ORDER BY T1.사업장코드, " & IIf(optPrtChk0.Value = True, "T1.매출처코드", "T3.매출처명") & ", 일자, 구분 "
    P_adoRec.CursorLocation = adUseClient
    strSQL = "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  & "T1.매출처코드 AS 매출처코드, T3.매출처명 AS 매출처명, " _
                  & "(T1.마감년월 + '00') AS 일자, '(년 이 월)' AS 적요, " _
    ...
    strSQL = strSQL & "UNION ALL " _
           & "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  ...
                  & "T1.입출고구분 AS 구분, (SUM(T1.출고수량 * T1.출고단가) * " & (PB_curVatRate + 1) & ") AS 출고금액, " _
                  & "(SELECT ISNULL(SUM(T4.미수금입금금액), 0) " _
                     & "FROM 미수금내역 T4 " _
                    & "WHERE T4.사업장코드 = T1.사업장코드 And T4.매출처코드 = T1.매출처코드 " _
                      & "AND T4.미수금입금일자 = T1.입출고일자 AND T1.입출고구분 = 1) AS 입금금액, " _
                  & "결제방법 = CASE WHEN T1.현금구분 = 1 THEN '현금' ELSE '외상' END,  '' AS 만기일자, '' AS 어음번호 " _
             & "FROM 자재입출내역 T1 " _
             & "LEFT JOIN 사업장 T2 ON T2.사업장코드 = T1.사업장코드 " _
             & "LEFT JOIN 매출처 T3 ON T3.사업장코드 = T1.사업장코드 AND T3.매출처코드 = T1.매출처코드 " _
    ...
    strSQL = strSQL & "UNION ALL " _
           & "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
             ...
             & "" & strWhere & " AND T1.미수금입금일자 " _
                         & "BETWEEN '" & (Mid(DTOS(dtpF_Date.Value), 1, 6) + "01") & "' AND '" & DTOS(dtpT_Date.Value) & "' " _
              & "AND NOT EXISTS (SELECT T4.입출고일자 " _
                                & "FROM 자재입출내역 T4 " _
                               & "WHERE T4.사업장코드 = T1.사업장코드 AND T4.매출처코드 = T1.매출처코드 " _
                                 & "AND T4.입출고일자 = T1.미수금입금일자 AND T4.입출고구분 = 1 AND T4.사용구분 = 0) " _
            & "GROUP BY T1.사업장코드, T2.사업장명, T1.매출처코드, T3.매출처명, " _
                     & "T1.미수금입금일자, T1.적요, T1.결제방법, T1.만기일자, T1.어음번호 "
    strSQL = strSQL _
           & "" & strOrderBy & " "
    On Error GoTo ERROR_TABLE_SELECT
    P_adoRec.Open strSQL, PB_adoCnnSQL, adOpenStatic, adLockReadOnly
...
Dim lngR      As Long
Dim curJanMny As Currency '입금가능금액
    'If Not (chkTotal.Value = 0) Then '전체로는 입금 불가
    '   lngC = -1
    '   Exit Function
    'End If
    For lngC = Text1.LBound To Text1.UBound
...
Dim strForPrtDateTime      As String  '출력일시           (Formula)
    ...
    With CrystalReport1
         If PB_Test = 0 Then
            strExeFile = App.Path & ".\미수금원장.rpt"
         Else
            strExeFile = App.Path & ".\미수금원장T.rpt"
         End If
         ...
         Else
            ...
            .Formulas(4) = "ForMSGbn = '" & PB_regUserinfoU.UserMSGbn & "'"                                '미수금발생구분
            '--- [Parameter Fileds] ---
            '지점코드
            ...
            Else
               .StoredProcParam(4) = " "
            End If
            .StoredProcParam(5) = CInt(PB_regUserinfoU.UserMSGbn)                 '미수금발생구분(1.전표, 2.계산서)
            '.StoredProcParam(6) = IIf(optPrtChk0.Value = True, 0, 1)             '정렬순서(0.매출처코드, 1.매출처명)
         End If
         If intRetCHK = 99 Then
            ...
            .WindowTitle = PB_regUserinfoU.UserBranchName & " - " & "미수금원장"
            .Action = 1
            .Reset
         End If
    End With
