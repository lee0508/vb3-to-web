...
Dim strBuffer          As String
    ...
    frmMain.SBar.Panels(4).Text = "매출세금계산서장부를 수정/삭제하여도 미지급금 지급내역에도 반영되지 않습니다. "
...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매입세금계산서장부(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
    ...
    With vsfg1
         If Row >= .FixedRows Then
            ...
            ElseIf _
               (Col = 11) Then  '품목및규격
               If .TextMatrix(Row, Col) <> .EditText Then
                  If .Cell(flexcpChecked, Row, 19) = flexChecked Then '미지급금이면(거래내역 있으면)
...
Dim strJukyo       As String  '자재입출내역의 적요
    If vsfg1.Row >= vsfg1.FixedRows Then
       With vsfg1
            ...
            End If
       End With
       intRetVal = MsgBox("수정된 매입세금계산서장부를 저장하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton1, "매입세금계산서장부 저장")
...
Dim lngLogCnt  As Long
    If (vsfg1.Row >= vsfg1.FixedRows And vsfg1.TextMatrix(vsfg1.Row, 20) = "정상") Then
       intRetVal = MsgBox("매입세금계산서장부에서 삭제하시겠습니까 ?", vbCritical + vbYesNo + vbDefaultButton2, "매입세금계산서장부 삭제")
       If intRetVal = vbNo Then
          vsfg1.SetFocus
          Exit Sub
       End If
       ...
       With vsfg1
            ...
            PB_adoCnnSQL.BeginTrans
            If intRetVal = vbYes Then  '매입세금계산서장부 삭제
               strSQL = "UPDATE 매입세금계산서장부 SET " _
                             & "사용구분 = 9, " _
                             & "수정일자 = '" & PB_regUserinfoU.UserServerDate & "', " _
                             & "사용자코드 = '" & PB_regUserinfoU.UserCode & "' " _
...
Dim lngRR      As Long
    ...
    If Len(Text1(0).Text) > 0 Then
       strWhere = "AND T1.매입처코드 = '" & Trim(Text1(0).Text) & "' "
    End If
    ...
    If cboCredit.ListIndex > 0 Then
       strWhere = strWhere + "AND T1.미지급구분 = " & (cboCredit.ListIndex - 1) & " "
    End If
...
Dim lngSeqNo             As Long     '세금계산서 일련번호
    ...
    strSQL = "SELECT T1.사업장코드 AS 사업장코드, T1.작성년도 AS 작성년도, T1.책번호 AS 책번호, T1.일련번호 AS 일련번호, " _
                  ...
                  & "T1.금액구분 AS 금액구분, T1.영청구분 AS 영청구분, T1.미지급구분 AS 미지급구분 " _
             & "FROM 세금계산서 T1 " _
