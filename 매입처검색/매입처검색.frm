...
Private Sub Form_Load()
    P_blnActived = False
    txtCode.Text = PB_strSupplierCode: txtName.Text = PB_strSupplierName
    If Len(PB_strSupplierCode) = 0 And Len(PB_strSupplierName) = 0 Then
       P_intFindGbn = 0    '수동검색
    ElseIf _
       Len(PB_strSupplierCode) <> 0 And Len(PB_strSupplierName) = 0 Then
         ...
         If .Row >= .FixedRows Then
            If KeyCode = vbKeyReturn Then
               If .Row = 0 Then
                  ...
                  PB_strSupplierName = ""
               Else
                  PB_strSupplierCode = .TextMatrix(.Row, 0)
                  PB_strSupplierName = .TextMatrix(.Row, 1)
                  If (PB_strFMCCallFormName = "frm매입작성2") Then
                     With frm매입작성2
                          .txtTelNo.Text = vsfg1.TextMatrix(vsfg1.Row, 3)    '전화번호
                          .txtAddress.Text = vsfg1.TextMatrix(vsfg1.Row, 2)  '주소
                     End With
                  End If
               End If
               Unload Me
...
Dim lngC    As Long
    With vsfg1              'Rows 1, Cols 4, RowHeightMax(Min) 300
         ...
         .RowHeightMax = 300
         .RowHeightMin = 300
         .ColWidth(0) = 1000   '매입처코드
         .ColWidth(1) = 2500   '매입처명
         .ColWidth(2) = 5000   '주소
         .ColWidth(3) = 2000   '전화번호
...
Dim lngRRR     As Long
    ...
    If P_intFindGbn = 1 Then
       strWhere = strWhere + IIf(Trim(strWhere) = "", "WHERE ", "AND ") _
                          & "T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
                      & "AND T1.매입처코드 Like '%" & Trim(txtCode.Text) & "%' " _
                      & "AND T1.사용구분 = 0 "
       strOrderBy = "ORDER BY T1.매입처코드 "
    ElseIf _
       P_intFindGbn = 2 Then
       strWhere = strWhere + IIf(Trim(strWhere) = "", "WHERE ", "AND ") _
                          & "T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
                      & "AND T1.매입처명 Like '%" & Trim(txtName.Text) & "%' " _
                      & "AND T1.사용구분 = 0 "
       strOrderBy = "ORDER BY T1.매입처명 "
    ElseIf _
       P_intFindGbn = 3 Then
       strWhere = strWhere + IIf(Trim(strWhere) = "", "WHERE ", "AND ") _
                          & "T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
    ...
    strSQL = "SELECT T1.매입처코드 AS 매입처코드, T1.매입처명 AS 매입처명, " _
                  & "(T1.주소 + SPACE(1) + T1.번지) AS 주소, T1.전화번호 AS 전화번호 " _
             & "FROM 매입처 T1 " _
             & "" & strWhere & " " _
             & "" & strOrderBy & " "
    ...
    Else
       With vsfg1
            ...
            Do Until P_adoRec.EOF
               ...
               .Cell(flexcpData, lngR, 0, lngR, 0) = P_adoRec("매입처코드")
               .TextMatrix(lngR, 1) = IIf(IsNull(P_adoRec("매입처명")), "", P_adoRec("매입처명"))
               .TextMatrix(lngR, 2) = IIf(IsNull(P_adoRec("주소")), "", P_adoRec("주소"))
               .TextMatrix(lngR, 3) = IIf(IsNull(P_adoRec("전화번호")), "", P_adoRec("전화번호"))
               If P_intFindGbn = 1 Then
                  If PB_strSupplierCode = Trim(.TextMatrix(lngR, 0)) Then
                     lngRR = lngR
                  End If
               ElseIf _
                  P_intFindGbn = 2 Then
                  If PB_strSupplierName = Trim(.TextMatrix(lngR, 1)) Then
                     lngRR = lngR
                  End If
               ElseIf _
                  P_intFindGbn = 3 Then
                  If PB_strSupplierCode = Trim(.TextMatrix(lngR, 0)) And _
                     PB_strSupplierName = Trim(.TextMatrix(lngR, 1)) Then
                     lngRR = lngR
                  End If
               End If
               P_adoRec.MoveNext
