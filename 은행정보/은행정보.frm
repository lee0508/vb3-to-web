...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "은행(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
...
Dim strSQL As String
    With Text1(Index)
         Select Case Index
                Case 0  '은행코드
                     .Text = Format(Vals(Trim(.Text)), "00")
                     If Not (.Text >= "01" And .Text <= "99") Then
                        .Text = ""
                     End If
                     If .Enabled = True Then
                        P_adoRec.CursorLocation = adUseClient
                        strSQL = "SELECT * FROM 은행 " _
                                & "WHERE 은행코드 = '" & Trim(.Text) & "' "
...
Dim intRetVal As Integer
    ...
    With vsfg1
         Screen.MousePointer = vbHourglass
         P_adoRec.CursorLocation = adUseClient
         If Text1(Text1.LBound).Enabled = True Then '은행 추가면 검색
            strSQL = "SELECT * FROM 은행 " _
                    & "WHERE 은행코드 = '" & Trim(Text1(Text1.LBound).Text) & "' "
            On Error GoTo ERROR_TABLE_SELECT
            P_adoRec.Open strSQL, PB_adoCnnSQL, adOpenStatic, adLockReadOnly
            If P_adoRec.RecordCount <> 0 Then
               P_adoRec.Close
         ...
         If Text1(Text1.LBound).Enabled = True Then '은행 추가
            strSQL = "INSERT INTO 은행(은행코드, 은행명, 적요, 사용구분," _
                                    & "수정일자, 사용자코드 ) Values( " _
                    & "'" & Trim(Text1(0).Text) & "','" & Trim(Text1(1).Text) & "', " _
                    & "'" & Trim(Text1(2).Text) & "', " & Val(Trim(Text1(3).Text)) & ", " _
         ...
         Else                                          '은행 변경
            strSQL = "UPDATE 은행 SET " _
                          & "은행명 = '" & Trim(Text1(1).Text) & "', " _
                          & "적요 = '" & Trim(Text1(2).Text) & "', " _
                          & "사용구분 = " & Val(Text1(3).Text) & ", " _
...
ERROR_TABLE_INSERT:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "은행 추가 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_UPDATE:
    PB_adoCnnSQL.RollbackTrans
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "은행 변경 실패"
    Unload Me
    Exit Sub
End Sub
    ...
    With vsfg1
         If .Row >= .FixedRows Then
            ...
            If intRetVal = vbYes Then
               ...
               'P_adoRec.Close
               If lngCnt <> 0 Then
                  MsgBox "XXX(" & Format(lngCnt, "#,#") & "건)가 있으므로 삭제할 수 없습니다.", vbCritical, "은행 삭제 불가"
               ...
               PB_adoCnnSQL.BeginTrans
               P_adoRec.CursorLocation = adUseClient
               strSQL = "DELETE FROM 은행 WHERE 은행코드 = " & .TextMatrix(.Row, 0) & " "
...
ERROR_TABLE_DELETE:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "은행 삭제 실패"
    cmdDelete.Enabled = True
    vsfg1.SetFocus
    'Unload Me
    Screen.MousePointer = vbDefault
...
Dim lngR    As Long
Dim lngC    As Long
    Text1(Text1.LBound).Enabled = False                '은행코드 FLASE
    With vsfg1              'Rows 0, Cols 16, RowHeightMax(Min) 300
    ...
    Else
       With vsfg1
            ...
            Do Until P_adoRec.EOF
               lngR = lngR + 1
               .TextMatrix(lngR, 0) = P_adoRec("은행코드")
               .Cell(flexcpData, lngR, 0, lngR, 0) = Trim(.TextMatrix(lngR, 0)) 'FindRow 사용을 위해
               .TextMatrix(lngR, 1) = IIf(IsNull(P_adoRec("은행명")), "", P_adoRec("은행명"))
               .TextMatrix(lngR, 2) = IIf(IsNull(P_adoRec("적요")), "", P_adoRec("적요"))
               .TextMatrix(lngR, 3) = IIf(IsNull(P_adoRec("사용구분")), "", P_adoRec("사용구분"))
               Select Case .ValueMatrix(lngR, 3)
                      Case 0
                           .TextMatrix(lngR, 4) = "정상"
                      Case 9
                           .TextMatrix(lngR, 4) = "사용불가"
                      Case Else
                           .TextMatrix(lngR, 4) = "구분오류"
               End Select
