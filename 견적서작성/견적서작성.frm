...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적서 작성(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
...
Private Sub vsfg1_MouseDown(Button As Integer, Shift As Integer, x As Single, Y As Single)
    With vsfg1
         If .Row >= .FixedRows Then
            If Len(.TextMatrix(.Row, 0)) <> 0 Then '0.자재코드, 3.수량, 7.계산
               If (.Col = 3) Then   '3. 수량
                  If Button = vbLeftButton Then
                     .Select .Row, .Col
                     .EditCell
               ...
               If (Col = 3) Then         '수량
                  If .TextMatrix(Row, Col) <> .EditText Then
                     ...
                     Else
                        ...
                        .TextMatrix(Row, 15) = Vals(.EditText) * .ValueMatrix(Row, 12)  '합계금액 = 수량 * 단가
                     End If
                  End If
               ElseIf _
                  (Col = 12) Then '12.출고단가
                  If .TextMatrix(Row, Col) <> .EditText Then                            '변경인 경우 입력금액 검사
                     ...
                     Else
                        blnModify = True
                        .TextMatrix(Row, 13) = Fix(Vals(.EditText) * (PB_curVatRate))               '부가세
                        curTmpMny = .ValueMatrix(Row, 15)
                        .TextMatrix(Row, 14) = Vals(.EditText) + .ValueMatrix(Row, 13)
                        .TextMatrix(Row, 15) = .ValueMatrix(Row, 3) * Vals(.EditText)   '합계금액 = 수량 * 단가
                     End If
                  End If
               ElseIf _
                  (Col = 13) Then '13.출고부가
    ...
    With vsfg1
         If .Row >= .FixedRows Then     '내역시세검색
            If KeyCode = vbKeyF2 And (Len(Text1(0).Text) > 0) And (Len(.TextMatrix(.Row, 0)) > 0) Then
               PB_strFMCCallFormName = "frm견적서작성"
    ...
    With vsfg1
         If .Row >= .FixedRows Then
            ...
            ElseIf _
               ...
               intRetVal = MsgBox("입력한 견적내역을 삭제하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton2, "발주내역삭제")
               If intRetVal = vbYes Then
                  lblTotMny.Caption = Format(Vals(lblTotMny.Caption) - .ValueMatrix(.Row, 15), "#,#.00")
...
Dim strMS         As String
    ...
    If Len(Text1(0).Text) < 1 Then '매출처코드
       ...
       Exit Sub
    End If
    If Vals(Text1(2).Text) < 0 Then '유효일수
       Text1(2).SetFocus
       Exit Sub
    End If
    If Not (LenH(Text1(3).Text) <= 50) Then '적요
       Text1(3).SetFocus
       Exit Sub
    End If
    With vsfg1
    ...
    End With
L_Title:
    strTitle = InputBox("견적서 제목을 입력하세요.", "견적서 제목", "")
    If Not (LenH(strTitle) <= 30) Then '적요
       GoTo L_Title
    End If
    intRetVal = MsgBox("입력된 자료" & lngChkCnt & "(건)을 저장(견적)하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton1, "자료 저장")
    ...
    P_adoRec.CursorLocation = adUseClient
    strSQL = "spLogCounter '견적', '" & PB_regUserinfoU.UserBranchCode + PB_regUserinfoU.UserClientDate & "', " _
    ...
    P_adoRec.Close
    '견적 INSERT
    strSQL = "INSERT INTO 견적(사업장코드, 견적일자, " _
                            & "견적번호, 매출처코드, " _
                            & "출고희망일자, 결제방법, " _
                            & "결제예정일자, 유효일수, " _
                            & "제목, 적요, 상태코드, " _
         ...
         For lngR = 1 To .Rows - 1
             ...
             If Len(.TextMatrix(lngR, 0)) > 0 Then 'And .ValueMatrix(lngR, 3) <> 0 Then
                ...
                strSQL = "INSERT INTO 견적내역(사업장코드, 견적일자, " _
                    ...
                    & "'', " & .ValueMatrix(lngR, 3) & ", " _
                    & "" & IIf(.Cell(flexcpChecked, lngR, 7, lngR, 7) = flexUnchecked, 0, 1) & ", '" & Text1(0).Text & "', " _
...
ERROR_TABLE_INSERT:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적내역 추가 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_UPDATE:
    PB_adoCnnSQL.RollbackTrans
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적내역 저장 실패"
    Unload Me
    Exit Sub
...
Dim lngC    As Long
    With vsfg1              'Rows 101, Cols 17, RowHeightMax(Min) 300
         ...
         .TextMatrix(0, 12) = "견적단가"     '
         .TextMatrix(0, 13) = "견적부가"     'H
         .TextMatrix(0, 14) = "견적가격"     'H
         .TextMatrix(0, 15) = "견적금액"     '
         .TextMatrix(0, 16) = "적요"
         ...
         Else
            ...
            If optPrtChk1.Value = True Then '견적서
               strSQL = "SELECT SUM(T1.출고단가 * T1.수량) AS 금액 " _
                        & "FROM 견적내역 T1 " _
                       & "WHERE T1.사업장코드 = '" & subBranchCode & "' " _
                      & "AND T1.견적일자 = '" & subOrderDate & "' " _
                      & "AND T1.견적번호 = " & subOrderNo & " " _
                      & "AND T1.상태코드 = 1 AND T1.사용구분 = 0 "
               On Error GoTo ERROR_TABLE_SELECT
               P_adoRec.Open strSQL, PB_adoCnnSQL, adOpenStatic, adLockReadOnly
               If P_adoRec.RecordCount = 0 Then
                  P_adoRec.Close
            ...
            '--- Parameter Fields ---
            .StoredProcParam(0) = subBranchCode                        '지점코드
            .StoredProcParam(1) = subOrderDate                         '견적일자
            .StoredProcParam(2) = subOrderNo                           '견적번호
            '0.견적의뢰서, 1.견적서
            .StoredProcParam(3) = IIf(optPrtChk1.Value = True, 1, 0)
         End If
         If intRetCHK = 99 Then
            .Connect = PB_adoCnnSQL.ConnectionString
            .CopiesToPrinter = 1         '출력매수
