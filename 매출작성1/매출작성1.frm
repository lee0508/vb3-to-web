...
Dim strBuffer          As String
    ...
    frmMain.SBar.Panels(4).Text = "현금인 경우 수금처리(매출거래처별수금처리)하시고, 계산서발행시 매출세금계산서장부에도 저장됩니다."
...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매출작성1(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
    ...
    With vsfg2
         If Row >= .FixedRows Then
            ...
            If blnModify = True Then
               ...
               Select Case Col
                      Case 16, 23, 24
                           vsfg1.TextMatrix(vsfg1.Row, 7) = vsfg1.ValueMatrix(vsfg1.Row, 7) - curTmpMny + .ValueMatrix(Row, 26)
         ...
         If (.Row >= .FixedRows) Then      '내역시세검색
            ...
            If KeyCode = vbKeyF2 And (Len(vsfg1.TextMatrix(vsfg1.Row, 5)) > 0) And _
              (Len(.TextMatrix(.Row, 4)) > 0) Then
               PB_strFMCCallFormName = "frm매출작성1"
    ...
    With vsfg2
         ...
         If .Row >= .FixedRows Then
            ...
            If KeyCode = vbKeyF1 Then  '자재시세검색
               ...
               'End If
            ElseIf _
               KeyCode = vbKeyInsert And cmdSave.Enabled = True Then '견적내역 추가
               .AddItem "", .Row + 1
               .Row = .Row + 1
               .TopRow = .Row
               .TextMatrix(.Row, 0) = .ValueMatrix(.Row - 1, 0) + 1 '순번
               .TextMatrix(.Row, 1) = .TextMatrix(.Row - 1, 1)   '사업장코드
               .TextMatrix(.Row, 2) = .TextMatrix(.Row - 1, 2)   '견적일자
               .TextMatrix(.Row, 3) = .TextMatrix(.Row - 1, 3)   '견적번호
               .TextMatrix(.Row, 8) = vsfg1.TextMatrix(vsfg1.Row, 5) '견적의 매출처코드
               .TextMatrix(.Row, 9) = vsfg1.TextMatrix(vsfg1.Row, 6) '견적의 매출처명
               ...
               If .Row <= vsfg1.ValueMatrix(vsfg1.Row, 16) Then  '견적번호의 마지막 아니면
               ...
               End If
               .TextMatrix(.Row, 29) = strTime                   '견적시간
               PB_strFMCCallFormName = "frm매출작성1"
               PB_strMaterialsCode = .TextMatrix(.Row, 4)
               PB_strMaterialsName = .TextMatrix(.Row, 5)
               PB_strSupplierCode = .TextMatrix(.Row, 8)
               ...
               End If
            ElseIf _
               KeyCode = vbKeyDelete And .Col = 9 Then  '매출처 지움(Not Used)
            ...
            ElseIf _
               ...
               intRetVal = MsgBox("입력한 견적내역을 삭제하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton2, "견적내역삭제")
               If intRetVal = vbYes Then
                  .TextMatrix(.Row, 28) = "D": .TextMatrix(.Row, 0) = "0"
                  vsfg1.TextMatrix(vsfg1.Row, 7) = vsfg1.ValueMatrix(vsfg1.Row, 7) - .ValueMatrix(.Row, 26)
...
Dim strMakeYear    As String '작성년도
...
Dim strSS          As String
Dim strMS          As String
Dim intChkCash     As Integer '1.현금매출
    ...
    If vsfg1.Row >= vsfg1.FixedRows Then
       intRetVal = MsgBox("견적서에서 적성한 자료를 매출처리하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton1, "견적서매출저장")
       If intRetVal = vbNo Then
          vsfg1.SetFocus
          Exit Sub
       End If
       intRetVal = MsgBox("현금매출을 하시겠습니까 ?", vbQuestion + vbYesNoCancel + vbDefaultButton2, "현금매출")
       If intRetVal = vbYes Then
          intChkCash = 1
       ElseIf _
       ...
       With vsfg2
            ...
            For lngRR = vsfg1.ValueMatrix(vsfg1.Row, 15) To vsfg1.ValueMatrix(vsfg1.Row, 16)
                If (.TextMatrix(lngRR, 28) = "I") Then 'And .ValueMatrix(lngRR, 16) <> 0) Then '견적내역 추가
                   strSQL = "INSERT INTO 견적내역(사업장코드, 견적일자, " _
                                               & "견적번호, 견적시간, 자재코드, " _
                                               & "매입처코드, 수량, " _
                                               & "계산서발행여부, 매출처코드, " _
                                               & "입고단가, 입고부가, " _
                ...
                'ElseIf _
                '   (.TextMatrix(lngRR, 28) = "I" And .ValueMatrix(lngRR, 16) = 0) Then '견적내역 취소
                '   .TextMatrix(lngRR, 28) = "D"
                '   lngDelCnt = lngDelCnt + 1     '삭제할 Row수 계산
                ElseIf _
                   (.TextMatrix(lngRR, 28) = "D") Then '견적내역 취소
                   strSQL = "DELETE FROM 견적내역 " _
                           & "WHERE 사업장코드 = '" & .TextMatrix(lngRR, 1) & "' " _
                             & "AND 견적일자 = '" & DTOS(.TextMatrix(lngRR, 2)) & "' " _
                             & "AND 견적번호 = " & .ValueMatrix(lngRR, 3) & " " _
                             & "AND 견적시간 = '" & .TextMatrix(lngRR, 29) & "' " _
                             & "AND 자재코드 = '" & .TextMatrix(lngRR, 6) & "' "
                   lngDelCnt = lngDelCnt + 1     '삭제할 Row수 계산
                   On Error GoTo ERROR_TABLE_DELETE
                   PB_adoCnnSQL.Execute strSQL
                ElseIf _
                   (.TextMatrix(lngRR, 28) = "U") Then 'And .ValueMatrix(lngRR, 16) <> 0) Then '견적내역 변경
                   strSQL = "UPDATE 견적내역 SET " _
                                 & "자재코드 = '" & .TextMatrix(lngRR, 4) & "', " _
                                 & "수량 = " & .ValueMatrix(lngRR, 16) & ", " _
                                 & "계산서발행여부 = " & IIf(.Cell(flexcpChecked, lngRR, 18, lngRR, 18) = flexUnchecked, 0, 1) & ", " _
       ...
       With vsfg1
            '견적
            If (.ValueMatrix(.Row, 16) - .ValueMatrix(.Row, 15) + 1) = lngDelCnt Then '견적내역 모두 삭제
               strSQL = "DELETE FROM 견적 " _
                       & "WHERE 사업장코드 = '" & .TextMatrix(.Row, 2) & "' AND 견적일자 = '" & DTOS(.TextMatrix(.Row, 3)) & "' " _
                         & "AND 견적번호 = " & .ValueMatrix(.Row, 4) & " "
               On Error GoTo ERROR_TABLE_DELETE
               PB_adoCnnSQL.Execute strSQL
            ...
            If vsfg1.Row > 0 Then
               For lngRR = vsfg1.ValueMatrix(vsfg1.Row, 15) To vsfg1.ValueMatrix(vsfg1.Row, 16)
                If .RowHidden(lngRR) = False Then
                   If (.TextMatrix(lngRR, 28) = "U" And .ValueMatrix(lngRR, 16) <> 0) Then   '견적내역 변경
       ...
       With vsfg2
            '견적내역(색상 원위치)
            If vsfg1.Row > 0 Then '현재선택된 견적 Row를 해제
            ...
            If vsfg1.Row > 0 Then
               ...
               For lngRR = vsfg1.ValueMatrix(vsfg1.Row, 15) To vsfg1.ValueMatrix(vsfg1.Row, 16)
                   If vsfg2.RowHidden(lngRR) = False Then
                      ...
                      strSQL = "UPDATE 견적내역 SET " _
                                    & "상태코드 = 2, " _
                                    & "출고일자 = '" & PB_regUserinfoU.UserClientDate & "', " _
                      ...
                      strSQL = "INSERT INTO 자재입출내역(사업장코드, 분류코드, 세부코드, 입출고구분, " _
                             ...
                             & "" & chkTaxBillPrint.Value & ", " & intChkCash & ", 0, '" & Trim(.TextMatrix(lngRR, 27)) & "', " _
                             & "'" & strMakeYear & "', " & lngLogCnt1 & ", " & lngLogCnt2 & ", " _
                             & "0, '" & PB_regUserinfoU.UserServerDate & "', " _
                             & "'" & PB_regUserinfoU.UserCode & "', '' ) "
                             'IIf(.Cell(flexcpChecked, lngRR, 18, lngRR, 18) = flexUnchecked, 0, 1)
                      On Error GoTo ERROR_TABLE_INSERT
                      PB_adoCnnSQL.Execute strSQL
                      ...
                      If .ValueMatrix(lngRR, 23) > 0 And PB_intOAutoPriceGbn = 1 Then
                         strSQL = "sp자재최종단가갱신 '" & PB_regUserinfoU.UserBranchCode & "', " _
                                & "'" & Mid(.TextMatrix(lngRR, 4), 1, 2) & "', '" & Mid(.TextMatrix(lngRR, 4), 3) & "', 2, " _
                                 ...
                                 & "" & .ValueMatrix(lngRR, 23) & ", '" & PB_regUserinfoU.UserClientDate & "' "
                         On Error GoTo ERROR_STORED_PROCEDURE
                         PB_adoCnnSQL.Execute strSQL
                      End If
                   End If
               ...
               strSQL = "UPDATE 견적 SET " _
                       ...
                       & "WHERE 사업장코드 = '" & vsfg1.TextMatrix(vsfg1.Row, 2) & "' " _
                         & "AND 견적일자 = '" & DTOS(vsfg1.TextMatrix(vsfg1.Row, 3)) & "' " _
                         & "AND 견적번호 = " & vsfg1.ValueMatrix(vsfg1.Row, 4) & " "
               On Error GoTo ERROR_TABLE_UPDATE
               PB_adoCnnSQL.Execute strSQL
               '세금계산서 작성(1.세금계산서, 2.매출세금계산서장부)
               If (chkTaxBillPrint.Value = 1) And (cboPrinter.ListIndex >= 0) Then
                  '1.세금계산서
                         ...
                         & "SELECT T1.사업장코드 AS 사업장코드, T1.작성년도 AS 작성년도, " _
                               ...
                               & "(SELECT TOP 1 (S2.자재명 + SPACE(1) + S2.규격)  FROM 자재입출내역 S1 " _
                                 ...
                                 & "WHERE S1.사업장코드 = T1.사업장코드 " _
                                   & "AND S1.거래일자 = T1.거래일자 AND S1.거래번호 = " & lngLogCnt & " " _
                                   & "AND S1.입출고구분 = 2 AND S1.사용구분 = 0 AND S1.계산서발행여부 = " & chkTaxBillPrint.Value & " " _
                  ...
                  strSQL = strSQL + _
                          ...
                          & "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
                            & "AND T1.거래일자 = '" & PB_regUserinfoU.UserClientDate & "' AND T1.거래번호 = " & lngLogCnt & " " _
                            & "AND T1.입출고구분 = 2 AND T1.사용구분 = 0 AND T1.계산서발행여부 = " & chkTaxBillPrint.Value & " " _
       ...
       If (chkPrint.Value = 1) And (cboPrinter.ListIndex >= 0) Then
          SubPubPrint_DealBill p, PB_intPrtTypeGbn, PB_regUserinfoU.UserClientDate, lngLogCnt      '거래명세서 출력
       End If
       If (chkTaxBillPrint.Value = 1) And (cboPrinter.ListIndex >= 0) Then '세금계산서 출력
          SubPubPrint_TaxBill p, PB_intPrtTypeGbn, PB_regUserinfoU.UserBranchCode, Mid(PB_regUserinfoU.UserClientDate, 1, 4), lngLogCnt1, lngLogCnt2, _
                              0, "", "", ""
       End If
       Screen.MousePointer = vbDefault
    End If
...
Dim lngLogCnt  As Long
    If vsfg1.Row >= vsfg1.FixedRows Then
       ...
       intRetVal = MsgBox("견적서를 삭제하시겠습니까 ?", vbCritical + vbYesNo + vbDefaultButton2, "견적서 삭제")
       If intRetVal = vbNo Then
          vsfg1.SetFocus
          Exit Sub
       End If
            ...
            For lngRR = .ValueMatrix(.Row, 16) To .ValueMatrix(.Row, 15) Step -1
                strSQL = "UPDATE 견적내역 SET " _
                        ...
                        & "WHERE 사업장코드 = '" & vsfg2.TextMatrix(lngRR, 1) & "' " _
                          & "AND 견적일자 = '" & DTOS(vsfg2.TextMatrix(lngRR, 2)) & "' " _
                          & "AND 견적번호 = " & vsfg2.ValueMatrix(lngRR, 3) & " " _
                          & "AND 견적시간 = '" & vsfg2.TextMatrix(lngRR, 29) & "' " _
                          & "AND 자재코드 = '" & vsfg2.TextMatrix(lngRR, 6) & "' "
                On Error GoTo ERROR_TABLE_DELETE
                PB_adoCnnSQL.Execute strSQL
                vsfg2.RemoveItem lngRR
            Next lngRR
...
ERROR_TABLE_DELETE:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적내역 삭제 실패"
    Unload Me
    Exit Sub
End Sub
...
Dim lngC    As Long
    With vsfg1              'Rows 1, Cols 17, RowHeightMax(Min) 300
         ...
         .RowHeightMax = 300
         .RowHeightMin = 300
         .ColWidth(0) = 1200   '출고희망일자(납기예정일자)
         .ColWidth(1) = 1730   '사업장코드+견적일자+견적번호(KEY)
         .ColWidth(2) = 1000   '사업장코드 'Hidden
         .ColWidth(3) = 1200   '견적일자   'Hidden
         .ColWidth(4) = 1000   '견적번호   'Hidden
         .ColWidth(5) = 1200   '매출처코드
         .ColWidth(6) = 2200   '매출처명
         .ColWidth(7) = 1800   '결제금액
         .ColWidth(8) = 1200   '매출일자
         .ColWidth(9) = 900    '유효일수
         .ColWidth(10) = 800   '결제방법
         .ColWidth(11) = 1200  '결제예정일자
         .ColWidth(12) = 2800  '제목
         .ColWidth(13) = 5000  '적요
         .ColWidth(14) = 1200  '견적자명
         ...
         .TextMatrix(0, 0) = "납기예정일자"
         .TextMatrix(0, 1) = "견적번호"
         .TextMatrix(0, 2) = "사업장코드" 'H
         .TextMatrix(0, 3) = "견적일자"   'H
         .TextMatrix(0, 4) = "견적번호"   'H
...
Dim lngC    As Long
    With vsfg2              'Rows 1, Cols 30, RowHeightMax(Min) 300
         ...
         .RowHeightMin = 300
         .ColWidth(0) = 400    'No
         .ColWidth(1) = 1000   '사업장코드         'H
         .ColWidth(2) = 1200   '견적일자           'H
         .ColWidth(3) = 1000   '견적번호           'H
         .ColWidth(4) = 1900   '품목코드(변경후)
         .ColWidth(5) = 2600   '품명(변경후)
         .ColWidth(6) = 1900   '품목코드(변경전)   'H
         ...
         .ColWidth(12) = 2000  '사업장코드+견적일자+견적번호+자재코드+매출처코드+(KEY) 'H
         .ColWidth(13) = 1000  '매입처코드 'H
         .ColWidth(14) = 2500  '매입처명   'H
         .ColWidth(15) = 2200  '자재규격
         .ColWidth(16) = 1000  '수량
         .ColWidth(17) = 800   '견적단위
         .ColWidth(18) = 800   '계산서발행여부
         .ColWidth(19) = 1600  '매입단가   'H
         .ColWidth(20) = 1200  '매입부가   'H
         .ColWidth(21) = 1600  '매입가격(단가+부가) 'H
         .ColWidth(22) = 1700  '매입금액   'H
         .ColWidth(23) = 1600  '매출단가
         .ColWidth(24) = 1200  '매출부가   'H
         .ColWidth(25) = 1600  '매출가격(단가+부가) 'H
         .ColWidth(26) = 1700  '매출금액
         .ColWidth(27) = 5000  '적요
         .ColWidth(28) = 800   'SQL구분
         .ColWidth(29) = 1000  '견적시간   'H
         .Cell(flexcpFontBold, 0, 0, 0, .Cols - 1) = False
         .Cell(flexcpAlignment, 0, 0, 0, .Cols - 1) = flexAlignCenterCenter
         .TextMatrix(0, 0) = "No"
         .TextMatrix(0, 1) = "사업장코드"   'H
         .TextMatrix(0, 2) = "견적일자"     'H
         .TextMatrix(0, 3) = "견적번호"     'H
...
Dim lngRR      As Long
    vsfg1.Rows = 1
    If optDate(0).Value = True Then '견적일자 기준
       strWhere = "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
               & "AND T1.상태코드 = 1 AND T1.사용구분 = 0 " _
               & "AND (T1.견적일자 BETWEEN '" & DTOS(dtpF_Date.Value) & "' AND '" & DTOS(dtpT_Date.Value) & "' ) "
       strOrderBy = "ORDER BY T1.견적일자, T1.견적번호 "
    End If
    If optDate(1).Value = True Then   '납기예정일자 기준
       strWhere = "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
               & "AND T1.상태코드 = 1 AND T1.사용구분 = 0 " _
               & "AND (T1.출고희망일자 BETWEEN '" & DTOS(dtpF_Date.Value) & "' AND '" & DTOS(dtpT_Date.Value) & "' ) "
       strOrderBy = "ORDER BY T1.출고희망일자, T1.견적일자, 견적번호 "
    End If
    If optDate(2).Value = True Then   '출고일자 기준
       strWhere = "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
               & "AND T1.상태코드 = 2 AND T1.사용구분 = 0 " _
               & "AND (T1.출고일자 BETWEEN '" & DTOS(dtpF_Date.Value) & "' AND '" & DTOS(dtpT_Date.Value) & "' ) "
       strOrderBy = "ORDER BY T1.출고일자, T1.견적일자, T1.견적번호 "
    End If
    If Len(Trim(Text1(0).Text)) = 0 Then
       strWhere = strWhere
    Else
       strWhere = strWhere + IIf(Trim(strWhere) = "", "WHERE ", "AND ") _
               & "T1.매출처코드 = '" & Trim(Text1(0).Text) & "' "
    End If
    P_adoRec.CursorLocation = adUseClient
    strSQL = "SELECT T1.출고희망일자, T1.사업장코드, T1.견적일자, T1.견적번호, " _
                  & "T1.매출처코드, T2.매출처명, T1.출고일자 AS 출고일자, T1.유효일수 AS 유효일수, " _
    ...
    Else
       With vsfg1
            ...
            Do Until P_adoRec.EOF
               lngR = lngR + 1
               .TextMatrix(lngR, 0) = Format(P_adoRec("출고희망일자"), "0000-00-00")
               .TextMatrix(lngR, 1) = P_adoRec("사업장코드") & "-" & Format(P_adoRec("견적일자"), "0000/00/00") _
                                    & "-" & CStr(P_adoRec("견적번호"))
               ...
               .TextMatrix(lngR, 3) = Format(P_adoRec("견적일자"), "0000-00-00")
               .TextMatrix(lngR, 4) = IIf(IsNull(P_adoRec("견적번호")), 0, P_adoRec("견적번호"))
               .TextMatrix(lngR, 5) = IIf(IsNull(P_adoRec("매출처코드")), "", P_adoRec("매출처코드"))
               .TextMatrix(lngR, 6) = IIf(IsNull(P_adoRec("매출처명")), "", P_adoRec("매출처명"))
               .TextMatrix(lngR, 8) = IIf(IsNull(P_adoRec("출고일자")), "", Format(P_adoRec("출고일자"), "0000-00-00"))
...
ERROR_TABLE_SELECT:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적 읽기 실패"
    Unload Me
    Exit Sub
End Sub
...
Dim strSubTotal As String
    ...
    If optDate(0).Value = True Then '견적일자 기준
       strWhere = "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
               ...
               & "AND (T1.견적일자 BETWEEN '" & DTOS(dtpF_Date.Value) & "' AND '" & DTOS(dtpT_Date.Value) & "' ) "
       strOrderBy = "ORDER BY T1.견적일자, T1.견적번호, T2.견적시간 "
    End If
    If optDate(1).Value = True Then   '납기예정일자 기준
       strWhere = "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
               & "AND T1.상태코드 = 1 AND T1.사용구분 = 0 AND T2.상태코드 = 1 AND T2.사용구분 = 0 " _
               & "AND (T1.출고희망일자 BETWEEN '" & DTOS(dtpF_Date.Value) & "' AND '" & DTOS(dtpT_Date.Value) & "' ) "
       strOrderBy = "ORDER BY T1.출고희망일자, T1.견적일자, T1.견적번호, T2.견적시간 "
    End If
    If optDate(2).Value = True Then   '출고일자 기준
       strWhere = "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
               & "AND T1.상태코드 = 2 AND T1.사용구분 = 0 AND T2.상태코드 = 2 AND T2.사용구분 = 0 " _
               & "AND (T1.출고일자 BETWEEN '" & DTOS(dtpF_Date.Value) & "' AND '" & DTOS(dtpT_Date.Value) & "' ) "
       strOrderBy = "ORDER BY T1.출고일자, T1.견적일자, T1.견적번호, T2.견적시간 "
    End If
    ...
    End If
    P_adoRec.CursorLocation = adUseClient
    '& "T1.매입처코드, T4.매입처명, T5.규격 AS 자재규격, T2.수량, "
    strSQL = "SELECT T1.출고희망일자, T1.사업장코드, T1.견적일자, T1.견적번호, T2.견적시간, " _
                  & "T2.자재코드, ISNULL(T5.자재명,'') AS 자재명, " _
                  & "ISNULL(T2.매출처코드,'') AS 매출처코드, ISNULL(T3.매출처명,'') AS 매출처명, " _
                  & "T4.매입처명, T5.규격 AS 자재규격, T2.수량, " _
                  & "T5.단위 AS 자재단위, T2.계산서발행여부, T2.입고단가, T2.입고부가, " _
                  & "T2.출고단가 , T2.출고부가, T2.적요 AS 적요 " _
            & "FROM 견적 T1 " _
           & "INNER JOIN 견적내역 T2 " _
                   & "ON T2.사업장코드 = T1.사업장코드 AND T2.견적일자 = T1.견적일자 AND T2.견적번호 = T1.견적번호 " _
    ...
    Else
       With vsfg2
            ...
            Do Until P_adoRec.EOF
               ...
               .TextMatrix(lngR, 2) = Format(P_adoRec("견적일자"), "0000-00-00")
               .TextMatrix(lngR, 3) = IIf(IsNull(P_adoRec("견적번호")), 0, P_adoRec("견적번호"))
               ...
               .TextMatrix(lngR, 12) = P_adoRec("사업장코드") & "-" & Format(P_adoRec("견적일자"), "0000/00/00") _
                                     & "-" & CStr(P_adoRec("견적번호")) & "-" & P_adoRec("자재코드") & "-" & P_adoRec("매출처코드")
               ...
               .TextMatrix(lngR, 16) = Format(P_adoRec("수량"), "#,#")
               ...
               .TextMatrix(lngR, 29) = IIf(IsNull(P_adoRec("견적시간")), "", P_adoRec("견적시간"))
               'If .TextMatrix(lngR, 0) = PB_regUserinfoU.UserBranchCode Then
               '   lngRR = lngR
               'End If
               .RowHidden(lngR) = True
            ...
            For lngR = 1 To .Rows - 1
                ...
                For lngRRR = 1 To vsfg1.Rows - 1
                    If strCell = vsfg1.TextMatrix(lngRRR, 1) Then
                       ...
                       '견적 합계금액 계산
                       vsfg1.TextMatrix(lngRRR, 7) = vsfg1.ValueMatrix(lngRRR, 7) + .ValueMatrix(lngR, 26)
...
ERROR_TABLE_SELECT:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적내역 읽기 실패"
    Unload Me
    Exit Sub
End Sub
...
Dim strEMail               As String
    ...
    With CrystalReport1
         If PB_Test = 0 Then
            strExeFile = App.Path & ".\견적서.rpt"
         Else
            strExeFile = App.Path & ".\견적서T.rpt"
         End If
         ...
         Else
            ...
            End If
            '금액(한자, 숫자)
            If optPrtChk1.Value = True Then '견적서
               strSQL = "SELECT SUM(T1.출고단가 * T1.수량) AS 금액 " _
                        & "FROM 견적내역 T1 " _
                       & "WHERE T1.사업장코드 = '" & vsfg1.TextMatrix(vsfg1.Row, 2) & "' " _
                         & "AND T1.견적일자 = '" & DTOS(vsfg1.TextMatrix(vsfg1.Row, 3)) & "' " _
                         & "AND T1.견적번호 = " & vsfg1.ValueMatrix(vsfg1.Row, 4) & " " _
            ...
            End If
            .Formulas(9) = "ForOrderGbn = " & IIf(optPrtChk1.Value = True, 1, 0) & " " '0.견적의뢰서, 1.견적서
            .Formulas(10) = "ForEMail = '" & strEMail & "' "
            '--- Parameter Fields ---
            .StoredProcParam(0) = vsfg1.TextMatrix(vsfg1.Row, 2)       '지점코드
            .StoredProcParam(1) = DTOS(vsfg1.TextMatrix(vsfg1.Row, 3)) '견적일자
            .StoredProcParam(2) = vsfg1.ValueMatrix(vsfg1.Row, 4)      '견적번호
            '0.견적의뢰서, 1.견적서
         ...
         If intRetCHK = 99 Then
            ...
            .WindowTitle = PB_regUserinfoU.UserBranchName & " - " & IIf(optPrtChk1.Value = True, "견적서", "견적의뢰서")
            .Action = 1
            .Reset
         End If
    End With
