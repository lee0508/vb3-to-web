...
Private Sub txt_FMt_Keydown(KeyCode As Integer, Shift As Integer)
    If (KeyCode = vbKeyHome Or KeyCode = vbKeyReturn) And Len(Trim(txt_FMt.Text)) > 0 Then   '자재검색
       PB_strCallFormName = "frm매입처별단가조회"
...
Dim lngC    As Long
    With vsfg1              'Rows 1, Cols 6, RowHeightMax(Min) 300
         ...
         .ColWidth(2) = 2000   '매입처코드
         .ColWidth(3) = 4000   '매입처명
...
Dim lngRRR      As Long
    ...
    End With
    strOrderBy = "ORDER BY T1.사업장코드, " & IIf(optPrtChk0.Value = True, "T1.매입처코드 ", "T3.매입처명 ") & ", T1.입출고일자 DESC "
    P_adoRec.CursorLocation = adUseClient
    strSQL = "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  & "T1.매입처코드 AS 매입처코드, T3.매입처명 AS 매입처명, " _
                  & "T1.입출고일자 AS 입출고일자, T1.입고단가 AS 입고단가 " _
             & "FROM 자재입출내역 T1 " _
    ...
    strSQL = strSQL _
           & "GROUP BY T1.사업장코드, T2.사업장명," _
                    & "T1.매입처코드, T3.매입처명, T1.입출고일자, T1.입고단가 "
    strSQL = strSQL _
           & "" & strOrderBy & " "
    ...
    Else
       With vsfg1
            ...
            Do Until P_adoRec.EOF
               ...
               .TextMatrix(lngR, 2) = P_adoRec("매입처코드")
               .TextMatrix(lngR, 3) = P_adoRec("매입처명")
               .TextMatrix(lngR, 4) = Format(P_adoRec("입출고일자"), "0000-00-00")
               .TextMatrix(lngR, 5) = P_adoRec("입고단가")
               'FindRow 사용을 위해
