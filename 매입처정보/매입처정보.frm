...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매입처 정보(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
...
ERROR_TABLE_SELECT:
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매입처정보 읽기 실패"
    Unload Me
    Exit Sub
End Sub
...
Dim intRetVal As Integer
    ...
    With vsfg1
         Screen.MousePointer = vbHourglass
         P_adoRec.CursorLocation = adUseClient
         If Text1(Text1.LBound).Enabled = True Then '매입처 추가면 검색
            strSQL = "SELECT * FROM 매입처 " _
                    & "WHERE 사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
            ...
            If P_adoRec.RecordCount <> 0 Then
               ...
               Text1(Text1.LBound).SetFocus
               Screen.MousePointer = vbDefault
               MsgBox "이미 등록된 매입처입니다. 매입처코드를 다시 확인하여주세요!", vbCritical + vbOKOnly, "매입처 등록"
         ...
         If Text1(Text1.LBound).Enabled = True Then '매입처 추가
            strSQL = "INSERT INTO 매입처(사업장코드, 매입처코드, 매입처명, 사업자번호," _
                                      & "법인번호, 대표자명, 대표자주민번호," _
                                      & "개업일자," _
                                      & "우편번호, 주소, 번지," _
                                      & "업태, 업종, 전화번호," _
         ...
         Else                                          '매입처정보 변경
            strSQL = "UPDATE 매입처 SET " _
                          & "매입처명 = '" & Trim(Text1(1).Text) & "', " _
...
ERROR_TABLE_INSERT:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매입처 추가 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_UPDATE:
    PB_adoCnnSQL.RollbackTrans
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매입처 변경 실패"
    Unload Me
    Exit Sub
End Sub
    ...
    With vsfg1
         If .Row >= .FixedRows Then
            ...
            If intRetVal = vbYes Then
               ...
               'P_adoRec.Close
               If lngCnt <> 0 Then
                  MsgBox "XXX(" & Format(lngCnt, "#,#") & "건)가 있으므로 삭제할 수 없습니다.", vbCritical, "매입처 삭제 불가"
               ...
               PB_adoCnnSQL.BeginTrans
               P_adoRec.CursorLocation = adUseClient
               strSQL = "DELETE FROM 매입처 " _
                       & "WHERE 사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
                         & "AND 매입처코드 = '" & .TextMatrix(.Row, 0) & "' "
...
ERROR_TABLE_DELETE:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매입처 삭제 실패"
    cmdDelete.Enabled = True
    vsfg1.SetFocus
    'Unload Me
    Screen.MousePointer = vbDefault
...
Dim lngC    As Long
    ...
    With vsfg1              'Rows 1, Cols 26, RowHeightMax(Min) 300
         ...
         .RowHeightMax = 300
         .RowHeightMin = 300
         .ColWidth(0) = 850    '매입처코드
         .ColWidth(1) = 2550   '매입처명
...
Dim lngRR      As Long
    P_adoRec.CursorLocation = adUseClient
    strSQL = "SELECT T1.사업장코드 AS 사업장코드, T1.매입처코드 AS 매입처코드, " _
                  & "T1.매입처명 AS 매입처명, T1.사업자번호 AS 사업자번호, " _
         ...
         & "ORDER BY " & IIf(optPrtChk0.Value = True, "T1.매입처코드", "T1.매입처명") & " "
    On Error GoTo ERROR_TABLE_SELECT
    P_adoRec.Open strSQL, PB_adoCnnSQL, adOpenStatic, adLockReadOnly
    ...
    Else
       With vsfg1
            ...
            Do Until P_adoRec.EOF
               lngR = lngR + 1
               .TextMatrix(lngR, 0) = P_adoRec("매입처코드")
               .Cell(flexcpData, lngR, 0, lngR, 0) = Trim(.TextMatrix(lngR, 0)) 'FindRow 사용을 위해
               .TextMatrix(lngR, 1) = IIf(IsNull(P_adoRec("매입처명")), "", P_adoRec("매입처명"))
...
Dim strForPrtDateTime      As String  '출력일시           (Formula)
    ...
    With CrystalReport1
         ...
         Else
            ...
            .Formulas(2) = "ForSortGbn = 1"                           '정렬순서(0.매입처코드, 1.매입처명)  'Mot Used
            '--- Parameter Fields ---
            '프로그램실행일자
            .StoredProcParam(0) = PB_regUserinfoU.UserBranchCode '지점코드
            .StoredProcParam(1) = 0                              '사용구분                                 'Mot Used
            .StoredProcParam(2) = IIf(optPrtChk0.Value = True, 0, 1)             '정렬순서(0.매입처코드, 1.매입처명)
         End If
         If intRetCHK = 99 Then
            ...
            .WindowTitle = PB_regUserinfoU.UserBranchName & " - " & "매입처정보"
            .Action = 1
            .Reset
         End If
    End With
