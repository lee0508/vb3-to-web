...
Dim lngC    As Long
    With vsfg1              'Rows 1, Cols 4, RowHeightMax(Min) 300
         ...
         .RowHeightMax = 300
         .RowHeightMin = 300
         .ColWidth(0) = 1000   '매출처코드
         .ColWidth(1) = 2500   '매출처명
         .ColWidth(2) = 5000   '주소
...
Dim lngRRR     As Long
    ...
    If P_intFindGbn = 1 Then
       strWhere = strWhere + IIf(Trim(strWhere) = "", "WHERE ", "AND ") _
                      ...
                      & "AND T1.매출처코드 Like '%" & Trim(txtCode.Text) & "%' " _
                      & "AND T1.사용구분 = 0 "
       strOrderBy = "ORDER BY T1.매출처코드 "
    ElseIf _
       P_intFindGbn = 2 Then
       strWhere = strWhere + IIf(Trim(strWhere) = "", "WHERE ", "AND ") _
                          & "T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
                      & "AND T1.매출처명 Like '%" & Trim(txtName.Text) & "%' " _
                      & "AND T1.사용구분 = 0 "
       strOrderBy = "ORDER BY T1.매출처명 "
    ElseIf _
       P_intFindGbn = 3 Then
       strWhere = strWhere + IIf(Trim(strWhere) = "", "WHERE ", "AND ") _
                          & "T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
    ...
    strSQL = "SELECT T1.매출처코드 AS 매출처코드, T1.매출처명 AS 매출처명, " _
    ...
    Else
       With vsfg1
            ...
            Do Until P_adoRec.EOF
               ...
               .Cell(flexcpData, lngR, 0, lngR, 0) = P_adoRec("매출처코드")
