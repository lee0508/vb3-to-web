...
Begin VB.Form frm매출세금계산서장부입력 
   BorderStyle     =   1  '단일 고정
   Caption         =   "매출처미수금 장부 입력(매출세금계산서장부 입력)"
   ClientHeight    =   5790
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12255
   ...
   End
End
Attribute VB_Name = "frm매출세금계산서장부입력"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'+-------------------------------------------------------------------------------------------------------+
' 프로그램 제 목 : 매출세금계산서장부입력
...
Dim strBuffer          As String
    ...
    frmMain.SBar.Panels(4).Text = "장부에 입력하여도 세금계산서에는 반영되지 않으며, 3.외상미수금이 아닌경우 미수금 수금내역에 저장됩니다. "
...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매출세금계산서장부입력(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
...
Dim intKijang      As Integer '1.기장
    ...
    If blnOK = False Then
       ...
       Exit Sub
    End If
    intRetVal = MsgBox("매출세금계산서장부에 저장하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton1, "매출세금계산서장부 저장")
    ...
    '매출세금계산서장부 추가
    PB_adoCnnSQL.BeginTrans
    strSQL = "INSERT INTO 매출세금계산서장부(사업장코드, 작성일자, 작성시간, " _
                                          & "매출처코드, 품목및규격, 수량, " _
                                          & "공급가액, 세액, 금액구분, 영청구분, " _
    ...
    If (chkPrint.Value = 1) And (cboPrinter.ListIndex >= 0) Then  '세금계산서 출력
       SubPubPrint_TaxBill p, PB_intPrtTypeGbn, PB_regUserinfoU.UserBranchCode, "", 0, 0, _
                           1, DTOS(dtpP_Date.Value), strServerTime, Trim(Text1(1).Text)
...
Dim strSQL As String
    ...
    strSQL = "SELECT T1.매출처코드 AS 매출처코드, " _
            ...
            & "GROUP BY T1.매출처코드 " _
            & "UNION ALL "
    If PB_regUserinfoU.UserMJGbn = "1" Then  '미수금발생구분 1.전표이면
       strSQL = strSQL _
...
Dim strSQL As String
    Text1(3).Text = "0": Text1(4).Text = "": Text1(5).Text = "": Text1(6).Text = ""
    Screen.MousePointer = vbHourglass
    P_adoRec.CursorLocation = adUseClient
    strSQL = "SELECT ISNULL(SUM(ISNULL(출고수량, 0) * ISNULL(출고단가, 0)), 0) AS 세금계산서금액 " _
             & "FROM 자재입출내역 T1 " _
            & "WHERE T1.사업장코드 = '" & strBranchCode & "' AND T1.매출처코드 = '" & strSupplierCode & "' " _
              & "AND T1.입출고일자 BETWEEN '" & strF_Date & "' AND '" & strT_Date & "' " _
              & "AND T1.사용구분 = 0 AND T1.계산서발행여부 = 0 AND T1.현금구분 = 0 " _
              & "AND T1.입출고구분 = 2 "
    On Error GoTo ERROR_TABLE_SELECT
    P_adoRec.Open strSQL, PB_adoCnnSQL, adOpenStatic, adLockReadOnly
    If P_adoRec.RecordCount = 0 Then
