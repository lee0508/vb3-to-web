...
Begin VB.Form frm품목별매출수량조회 
   ...
   End
End
Attribute VB_Name = "frm품목별매출수량조회"
...
Private Sub txt_FMt_Keydown(KeyCode As Integer, Shift As Integer)
    If (KeyCode = vbKeyHome Or KeyCode = vbKeyReturn) And Len(Trim(txt_FMt.Text)) > 0 Then '자재검색
       PB_strCallFormName = "frm품목별매출수량조회"
...
Dim lngR    As Long
Dim lngC    As Long
    With vsfg1              'Rows 1, Cols 12, RowHeightMax(Min) 300
         ...
         .ColWidth(8) = 2000   '수량
         .ColWidth(9) = 1000   '단위
         .ColWidth(10) = 1400  '출고수량 * 출고단가
         .ColWidth(11) = 1400  '입고수량 * 입고단가
         ...
         .TextMatrix(0, 8) = "수량"
         .TextMatrix(0, 9) = "단위"
         .TextMatrix(0, 10) = "매출금액"   '수량 * 단가
         .TextMatrix(0, 11) = "매입금액"   '수량 * 단가
         .ColHidden(0) = True: .ColHidden(1) = True: .ColHidden(5) = True: .ColHidden(10) = True:: .ColHidden(11) = True
...
Dim lngRRR      As Long
    ...
    lblTotIMny.Caption = "": lblTotOMny.Caption = ""
    vsfg1.Rows = 1
    With vsfg1
         '검색조건 매출처
    ...
    strSQL = "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  ...
                  & "SUM(T1.출고수량) AS 출고수량, SUM(T1.출고수량 * T1.출고단가) AS 출고금액, " _
                  & "SUM(T1.출고수량 * T1.입고단가) AS 입고금액 " _
             & "FROM 자재입출내역 T1 " _
    ...
    'strSQL = "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  ...
                  & "ISNULL(T5.규격,'') AS 규격, ISNULL(T5.단위,'') AS 단위, " _
                  & "SUM(T1.출고수량) AS 출고수량, SUM(T1.출고수량 * T1.출고단가) AS 출고금액, " _
                  & "(SELECT ISNULL(SUM(ISNULL(입고수량, 0) * ISNULL(입고단가, 0)), 0) " _
                     & "FROM 자재입출내역 " _
                    & "WHERE 사업장코드 = T1.사업장코드 AND 분류코드 = T1.분류코드 AND 세부코드 = T1.세부코드 " _
                      & "AND 입출고구분 = 1 AND 사용구분 = 0 " _
                      & "AND 입출고일자 BETWEEN '" & DTOS(dtpF_Date.Value) & "' AND '" & DTOS(dtpT_Date.Value) & "' ) AS 입고금액 " _
             & "FROM 자재입출내역 T1 " _
             & "LEFT JOIN 사업장 T2 ON T2.사업장코드 = T1.사업장코드 " _
             & "LEFT JOIN 매출처 T3 ON T3.사업장코드 = T1.사업장코드 AND T3.매출처코드 = T1.매출처코드 " _
             & "LEFT JOIN 자재분류 T4 ON T4.분류코드 = T1.분류코드 " _
    ...
    Else
       With vsfg1
            ...
            Do Until P_adoRec.EOF
               ...
               .TextMatrix(lngR, 8) = P_adoRec("출고수량")
               .TextMatrix(lngR, 9) = P_adoRec("단위")
               .TextMatrix(lngR, 10) = P_adoRec("출고금액")
               .TextMatrix(lngR, 11) = P_adoRec("입고금액")
               'FindRow 사용을 위해
               ...
               lblTotIMny.Caption = Format((Vals(lblTotIMny.Caption) + .ValueMatrix(lngR, 11)), "#,#.00")
               P_adoRec.MoveNext
            Loop
            P_adoRec.Close
            '전체 합계
