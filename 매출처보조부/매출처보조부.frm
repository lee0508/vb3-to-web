...
Dim lngC    As Long
    With vsfg1              'Rows 1, Cols 17, RowHeightMax(Min) 300
         ...
         .ColWidth(11) = 1000  '출고수량(수량)
         .ColWidth(12) = 1400  '출고단가(공급가)
         .ColWidth(13) = 1700  '출고금액(부가세미포함)
         .ColWidth(14) = 1600  '출고부가세
         .ColWidth(15) = 1700  '출고금액(합계)
         .ColWidth(16) = 2000  '비고
         ...
         .TextMatrix(0, 10) = "구분"
         .TextMatrix(0, 11) = "수량"
         .TextMatrix(0, 12) = "매출단가"   '품목단가
         .TextMatrix(0, 13) = "매출금액"   '수량 * 단가
         .TextMatrix(0, 14) = "매출부가"   'H '((수량 * 단가) * (PB_curVatRate + 1)) - (수량 * 단가)
         .TextMatrix(0, 15) = "매출금액(VAT)" '(수량 * 단가) * (PB_curVatRate + 1)
         .TextMatrix(0, 16) = "비고"
...
Dim curTTUMny   As Currency  '전  체매출금액누계     '(단가 * 수량)
    ...
    End With
    strOrderBy = "ORDER BY T1.사업장코드, " & IIf(optPrtChk0.Value = True, "T1.매출처코드, T3.매출처명 ", "T3.매출처명, T1.매출처코드 ") & ", 일자, 자재명, 구분 "
    ...
    strSQL = "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  ...
                  & "0 AS 출고수량, 0 AS 출고단가, " _
                  & "0 AS 출고부가, (T1.미수금누계금액 - T1.미수금입금누계금액) AS 출고금액 " _
             & "FROM 미수금원장마감 T1 " _
             ...
             & "" & strWhere & " AND SUBSTRING(T1.마감년월,5,2) = '00' " _
              & "AND (T1.미수금누계금액 <> 0 OR T1.미수금입금누계금액 <> 0) " _
    ...
    strSQL = strSQL & "UNION ALL " _
           & "SELECT T1.사업장코드 AS 사업장코드, T2.사업장명 AS 사업장명, " _
                  ...
                  & "T1.세부코드 AS 세부코드, ISNULL(T5.자재명, '') AS 자재명, " _
                  & "ISNULL(T5.규격,'') AS 규격, ISNULL(T5.단위,'') AS 단위, T1.입출고구분 AS 구분, " _
                  & "SUM(T1.출고수량) AS 출고수량, T1.출고단가 AS 출고단가, " _
                  & "T1.출고부가 AS 출고부가, (SUM(T1.출고수량*T1.출고단가) * (" & PB_curVatRate + 1 & ")) AS 출고금액 " _
             & "FROM 자재입출내역 T1 " _
             & "LEFT JOIN 사업장 T2 ON T2.사업장코드 = T1.사업장코드 " _
             & "LEFT JOIN 매출처 T3 ON T3.사업장코드 = T1.사업장코드 AND T3.매출처코드 = T1.매출처코드 " _
    ...
    Else
       With vsfg1
            ...
            Do Until P_adoRec.EOF
               ...
               If lngR <> 2 Then    '처음 레코드 아니면
                  If .TextMatrix(lngR - 1, 1) <> P_adoRec("매출처코드") Then '매출처코드가 다르면
                     If .TextMatrix(lngR - 1, 3) <> "" Then '일 집계면
                        '해당월누계수량
                         ...
                         .TextMatrix(lngR, 13) = curMUMny         '월매출금액
                         .TextMatrix(lngR, 15) = (curMUMny * (PB_curVatRate + 1)) '월매출금액(VAT)
                     ...
                     .TextMatrix(lngR, 13) = curTUMny             '업체매출금액누계
                     .TextMatrix(lngR, 15) = (curTUMny * (PB_curVatRate + 1))     '업체매출금액누계(VAT)
                  ...
                  Else                                                          '매출처코드 같으면
                     If .TextMatrix(lngR - 1, 3) <> "" And _
                         Mid(StrDate, 1, 6) <> Mid(P_adoRec("일자"), 1, 6) Then '일 집계고 And 월이 다르면
                         '해당월입(출)고누계
                         .AddItem ""
                         '.TextMatrix(lngR, 4) = "(" & Format(Mid(StrDate, 1, 6), "0000-00") & Space(1) & "월계)"
                         .TextMatrix(lngR, 4) = "(월    계)"
               ...
               ElseIf _
                  P_adoRec("구분") = 2 Then
                  .TextMatrix(lngR, 10) = "매출"
               ElseIf _
                  P_adoRec("구분") = 8 Then
                  .TextMatrix(lngR, 10) = "임의"
               End If
               .TextMatrix(lngR, 11) = P_adoRec("출고수량")                          '수량
               .TextMatrix(lngR, 12) = P_adoRec("출고단가")                          '품목별 매출단가
               .TextMatrix(lngR, 13) = P_adoRec("출고수량") * P_adoRec("출고단가")   '해당행의 매출금액
               .TextMatrix(lngR, 14) = P_adoRec("출고금액") - .ValueMatrix(lngR, 13) '해당행의 매출부가세
               If Mid(P_adoRec("일자"), 7, 2) = "00" Then
                  curJanMny = curJanMny + P_adoRec("출고금액") '년(월)이월 누계
                  .TextMatrix(lngR, 15) = curJanMny
               Else
                  .TextMatrix(lngR, 15) = P_adoRec("출고금액")                       '해당행의 매출금액(부가세포함)
               End If
               If Mid(P_adoRec("일자"), 7, 2) <> "00" Then
                  curMQAmt = curMQAmt + P_adoRec("출고수량")
                  curMUMny = curMUMny + (P_adoRec("출고수량") * P_adoRec("출고단가"))
               End If
               '해당누계금액
               curTQAmt = curTQAmt + P_adoRec("출고수량")
               curTUMny = curTUMny + (P_adoRec("출고단가") * P_adoRec("출고수량"))
               curTTQAmt = curTTQAmt + P_adoRec("출고수량")
               curTTUMny = curTTUMny + (P_adoRec("출고단가") * P_adoRec("출고수량"))
               StrDate = P_adoRec("일자")
               'FindRow 사용을 위해
            ...
            If chkTotal.Value = 1 Then
               ...
               .TextMatrix(lngR, 13) = curTTUMny              '전체매출금액누계
               .TextMatrix(lngR, 15) = (curTTUMny * (PB_curVatRate + 1))      '전체매출금액누계(VAT)
...
ERROR_TABLE_SELECT:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매출처보조부 읽기 실패"
    Unload Me
    Exit Sub
End Sub
...
Dim strForPrtDateTime      As String  '출력일시           (Formula)
    ...
    With CrystalReport1
         If PB_Test = 0 Then
            strExeFile = App.Path & ".\매출처보조부.rpt"
         Else
            strExeFile = App.Path & ".\매출처보조부T.rpt"
         End If
         ...
         If intRetCHK = 99 Then
            ...
            .WindowTitle = PB_regUserinfoU.UserBranchName & " - " & "매출처보조부"
            .Action = 1
            .Reset
         End If
    End With
