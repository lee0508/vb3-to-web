...
Dim strBuffer          As String
    ...
    frmMain.SBar.Panels(4).Text = "매출일자 유의, 현금인경우 수금처리(매출거래처별수금처리)하시고, 계산서발행시 매출세금계산서장부에도 저장됩니다."
    If P_blnActived = False Then
       ...
       strDefaultPrinter = Trim(aryPrinter(0))
...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "매출작성2(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
...
Dim varRetVal  As Variant
    If (Index = 0 And (KeyCode = vbKeyHome Or KeyCode = vbKeyReturn)) Then  '매출처검색
       PB_strFMCCallFormName = "frm매출작성2"
...
Private Sub vsfg1_DblClick()
    With vsfg1
         If .Row >= .FixedRows Then
             vsfg1_KeyDown vbKeyF1, 0  '자재시세검색 바로 감.
         End If
    End With
End Sub
...
Private Sub vsfg1_MouseDown(Button As Integer, Shift As Integer, x As Single, Y As Single)
    With vsfg1
         If .Row >= .FixedRows Then
            If Len(.TextMatrix(.Row, 0)) <> 0 Then '0.자재코드, 3.수량, 7.계산서발행여부
               If (.Col = 3) Then   '3. 발주량
                  If Button = vbLeftButton Then
                     .Select .Row, IIf(.MouseCol = .Col, .MouseCol, .Col)
                     .EditCell
                  End If
               ElseIf _
                  (.Col = 7) Then   '7. 계산서발행여부
                  If Not (.ValueMatrix(.Row, 3) = 0) Then
                     If Button = vbLeftButton Then
                        .Select .Row, .Col
                        .EditCell
                     End If
                  End If
               ElseIf _
                  (.Col = 12 Or .Col = 13) Then   '12.출고단가, 13.출고부가
                  If Button = vbLeftButton Then
                     .Select .Row, .Col
                     .EditCell
                  End If
...
'         If .MouseRow >= .FixedRows Then
'            If Len(.TextMatrix(.Row, 0)) <> 0 Then '0.자재코드, 3.수량, 7.계산서발행여부
'               If (.MouseCol = 3 Or .Col = 3) Then   '3. 발주량
'                  If Button = vbLeftButton Then
'                     .Select .MouseRow, IIf(.MouseCol = .Col, .MouseCol, .Col)
'                     .EditCell
'                  End If
'               ElseIf _
'                  (.MouseCol = 7) Then   '7. 계산서발행여부
'                  If Not (.ValueMatrix(.Row, 3) = 0) Then
'                     If Button = vbLeftButton Then
'                        .Select .MouseRow, .MouseCol
'                        .EditCell
...
'                  End If
'               ElseIf _
'                  (.MouseCol = 12 Or .MouseCol = 13) Then   '12.출고단가, 13.출고부가
'                  If Button = vbLeftButton Then
'                     .Select .MouseRow, .MouseCol
'                     .EditCell
'                  End If
    ...
    With vsfg1
         If Row >= .FixedRows Then
            If Len(.TextMatrix(Row, 0)) <> 0 Then
               If (Col = 3) Then         '수량
                  If .TextMatrix(Row, Col) <> .EditText Then
                     ...
                     Else
                        blnModify = True
                        curTmpMny = .ValueMatrix(Row, 15)
                        .TextMatrix(Row, 15) = Vals(.EditText) * .ValueMatrix(Row, 12)
                     End If
                  End If
               ElseIf _
                  (Col = 12) Then '12.출고단가
                  If .TextMatrix(Row, Col) <> .EditText Then                            '변경인 경우 입력금액 검사
                     ...
                     Else
                        blnModify = True
                        .TextMatrix(Row, 13) = Fix(Vals(.EditText) * PB_curVatRate)
                        curTmpMny = .ValueMatrix(Row, 15)
                        .TextMatrix(Row, 14) = Vals(.EditText) + .ValueMatrix(Row, 13)
                        .TextMatrix(Row, 15) = .ValueMatrix(Row, 3) * Vals(.EditText)
                     End If
                  End If
               ElseIf _
                  (Col = 13) Then '13.출고부가
                  If .TextMatrix(Row, Col) <> .EditText Then                            '변경인 경우 입력금액 검사
                     If (IsNumeric(.EditText) = False Or Vals(.EditText) < 0 Or _
                        ...
                        (Vals(.EditText) > .ValueMatrix(Row, 12)) Then
                        Beep
                        Cancel = True
                     Else
                        blnModify = True
                        curTmpMny = .ValueMatrix(Row, 15)
                        .TextMatrix(Row, 14) = .ValueMatrix(Row, 12) + Vals(.EditText)
                        .TextMatrix(Row, 15) = .ValueMatrix(Row, 3) * .ValueMatrix(Row, 12)
                     End If
                  End If
               ElseIf _
            ...
            If blnModify = True Then
               Select Case Col
                      Case 3, 12, 13
                           lblTotMny.Caption = Format(Vals(lblTotMny.Caption) - curTmpMny + .ValueMatrix(Row, 15), "#,#.00")
                      Case Else
    ...
    With vsfg1
         If .Row >= .FixedRows Then
            If KeyCode = vbKeyF1 And Len(Trim(Text1(0).Text)) > 0 Then  '자재시세검색
               'If (.MouseCol = 1 Or .MouseCol = 2) Then
                  PB_strFMCCallFormName = "frm매출작성2"
                  PB_strMaterialsCode = .TextMatrix(.Row, 0)
                  PB_strMaterialsName = .TextMatrix(.Row, 1)
                  If Len(Trim(Text1(0).Text)) = 0 Then
            ...
            ElseIf _
               KeyCode = vbKeyDelete And (.Col <> 6) And (Len(.TextMatrix(.Row, 0)) > 0) Then
               intRetVal = MsgBox("입력한 매출내역을 삭제하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton2, "매출내역삭제")
               If intRetVal = vbYes Then
                  lblTotMny.Caption = Format(Vals(lblTotMny.Caption) - .ValueMatrix(.Row, 15), "#,#.00") '전체금액에서 제외
...
Dim lngLogCnt1     As Long    '책번호
Dim lngLogCnt2     As Long    '일련번호
...
Dim strJ_Date      As String  '작업일자
    ...
    If (vsfg1.Rows) = 1 Then
       Exit Sub
    End If
    If Len(Text1(0).Text) < 1 Then '매출처코드
       Text1(0).SetFocus
       Exit Sub
    End If
    With vsfg1
    ...
    P_adoRec.CursorLocation = adUseClient
    strSQL = "spLogCounter '자재입출내역', '" & PB_regUserinfoU.UserBranchCode + strJ_Date + "2" & "', " _
                            & "0, 0, '" & PB_regUserinfoU.UserServerDate & "', '" & PB_regUserinfoU.UserCode & "' "
    On Error GoTo ERROR_STORED_PROCEDURE
    P_adoRec.Open strSQL, PB_adoCnnSQL, adOpenStatic, adLockReadOnly
    lngLogCnt = P_adoRec(0)
    P_adoRec.Close
    If chkTaxBillPrint.Value = 1 And (Vals(lblTotMny.Caption) > 0) Then
         ...
         For lngR = 1 To .Rows - 1
             If Len(.TextMatrix(lngR, 0)) > 0 Then 'And .ValueMatrix(lngR, 3) <> 0 Then
                ...
                strSQL = "INSERT INTO 자재입출내역(사업장코드, 분류코드, " _
                             ...
                             & "'" & Mid(.TextMatrix(lngR, 0), 3) & "', 2, " _
                             & "'" & strJ_Date & "', '" & strTime & "', " _
                             & "0, " & .ValueMatrix(lngR, 8) & ", " _
                             & "" & .ValueMatrix(lngR, 9) & ", " & .ValueMatrix(lngR, 3) & ", " _
                             & "" & .ValueMatrix(lngR, 12) & ", " & .ValueMatrix(lngR, 13) & ", " _
                             & "'' , '" & Trim(Text1(0).Text) & "', " _
                             & "'" & strJ_Date & "', 0, '', 0, " _
                             & "'" & strJ_Date & "', " & lngLogCnt & ", " _
                             & "" & chkTaxBillPrint.Value & ", " & intChkCash & ", 0, '" & .TextMatrix(lngR, 16) & "', " _
                ...
                strSQL = "sp자재최종입출고일자갱신 '" & PB_regUserinfoU.UserBranchCode & "', " _
                       & "'" & Mid(.TextMatrix(lngR, 0), 1, 2) & "', '" & Mid(.TextMatrix(lngR, 0), 3) & "', 2 "
                On Error GoTo ERROR_STORED_PROCEDURE
                PB_adoCnnSQL.Execute strSQL
                '자재최종단가갱신(사업장코드, 분류코드, 세부코드, 입출고구분, 업체코드, 단가, 거래일자)
                If .ValueMatrix(lngR, 12) > 0 And PB_intOAutoPriceGbn = 1 Then
                   strSQL = "sp자재최종단가갱신 '" & PB_regUserinfoU.UserBranchCode & "', " _
                          & "'" & Mid(.TextMatrix(lngR, 0), 1, 2) & "', '" & Mid(.TextMatrix(lngR, 0), 3) & "', 2, " _
                          & "'" & Trim(Text1(0).Text) & "', " _
                          & "" & .ValueMatrix(lngR, 12) & ", '" & strJ_Date & "' "
                   On Error GoTo ERROR_STORED_PROCEDURE
                   PB_adoCnnSQL.Execute strSQL
                End If
             End If
    ...
    If (chkTaxBillPrint.Value = 1) And (cboPrinter.ListIndex >= 0) And (Vals(lblTotMny.Caption) <> 0) Then
       '1.세금계산서
       ...
       strSQL = strSQL + _
                     ...
                     & "1 AS 발행구분, 0 AS 작성구분, 1 AS 미수구분, '' AS 적요, 0 AS 사용구분, " _
               ...
               & "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
                 & "AND T1.거래일자 = '" & strJ_Date & "' AND T1.거래번호 = " & lngLogCnt & " " _
    ...
    If (chkPrint.Value = 1) And (cboPrinter.ListIndex >= 0) Then
       SubPubPrint_DealBill p, PB_intPrtTypeGbn, strJ_Date, lngLogCnt                           '거래명세서 출력
    End If
    If (chkTaxBillPrint.Value = 1) And (cboPrinter.ListIndex >= 0) Then '세금계산서 출력
       SubPubPrint_TaxBill p, PB_intPrtTypeGbn, PB_regUserinfoU.UserBranchCode, Mid(strJ_Date, 1, 4), lngLogCnt1, lngLogCnt2, _
                           0, "", "", ""
    End If
    Screen.MousePointer = vbDefault
...
Dim lngC As Long
    ...
    With vsfg1
         ...
         .Row = 1: .Col = 3
         .TopRow = 1: .LeftCol = 3
         .Cell(flexcpChecked, 1, 7, .Rows - 1, 7) = flexChecked 'flexUnchecked
         .Cell(flexcpText, 1, 7, .Rows - 1, 7) = "계 산"
    End With
End Sub
'+----------------------------------+
...
Dim lngC    As Long
    With vsfg1              'Rows 101, Cols 17, RowHeightMax(Min) 300
         ...
         .ColWidth(7) = 800    '계산서발행여부
         .ColWidth(8) = 1500   '입고단가     'H
         .ColWidth(9) = 1300   '입고부가     'H
         .ColWidth(10) = 1500  '입고가격(단가 + 부가) 'H
         .ColWidth(11) = 2000  '입고금액(수량 * 입고가격) 'H
         .ColWidth(12) = 1500  '출고단가
         .ColWidth(13) = 1300  '출고부가
         .ColWidth(14) = 1500  '출고가격(단가+부가) 'H
         .ColWidth(15) = 2000  '출고금액(수량 * 출고가격)
         .ColWidth(16) = 4500  '적요
         ...
         .TextMatrix(0, 7) = "계산"
         .TextMatrix(0, 8) = "매입단가"      'H
         .TextMatrix(0, 9) = "매입부가"      'H
         .TextMatrix(0, 10) = "매입가격"     'H
         .TextMatrix(0, 11) = "매입금액"     'H
         .TextMatrix(0, 12) = "매출단가"
         .TextMatrix(0, 13) = "매출부가"     'H
         .TextMatrix(0, 14) = "매출단가"     'H
         .TextMatrix(0, 15) = "매출금액"
         .TextMatrix(0, 16) = "적요"
         
         .ColHidden(5) = True: .ColHidden(6) = True: .ColHidden(7) = True
         .ColHidden(8) = True: .ColHidden(9) = True: .ColHidden(10) = True: .ColHidden(11) = True
         .ColHidden(13) = True: .ColHidden(14) = True
         .ColFormat(3) = "#,#"
         For lngC = 8 To 15
             .ColFormat(lngC) = "#,#.00"
         Next lngC
         ...
         '    .MergeCol(lngC) = True
         'Next lngC
         .Cell(flexcpChecked, 1, 7, .Rows - 1, 7) = flexChecked
         .Cell(flexcpText, 1, 7, .Rows - 1, 7) = "계 산"
