...
Dim SQL     As String
    ...
    frmMain.SBar.Panels(4).Text = "기존매입일자를 정확히 입력하여 주세요. "
...
Dim lngC As Long
    With vsfg1
         ...
         If .Row >= .FixedRows Then
            Text1(0).Text = .TextMatrix(.Row, 4): Text1(1).Text = .TextMatrix(.Row, 5) '매입처
            Text1(2).Text = .TextMatrix(.Row, 6): Text1(3).Text = .TextMatrix(.Row, 7) '자재
            Text1(4).Text = .TextMatrix(.Row, 8)                      '규격
            dtpAppDate.Value = Format(DTOS(.TextMatrix(.Row, 3)), "0000-00-00")        '입고일자
            Text1(5).Text = Format(.ValueMatrix(.Row, 9), "#,0")   '반품수량
            Text1(6).Text = .TextMatrix(.Row, 10)                     '단위
            Text1(7).Text = Format(.ValueMatrix(.Row, 11), "#,0.00")  '입고단가
            Text1(8).Text = Format(.ValueMatrix(.Row, 12), "#,0")     '입고부가
...
Dim intChkCash    As Integer
    ...
    '자재입출내역 검사
    strSQL = "SELECT ISNULL(SUM(T1.입고수량),0) AS 입고수량 " _
             & "FROM 자재입출내역 T1 " _
            & "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' " _
              & "AND (T1.분류코드 + T1.세부코드) = '" & Text1(2).Text & "' " _
              & "AND T1.매입처코드 = '" & Text1(0).Text & "' " _
              & "AND ((T1.입출고구분 = 1 AND T1.입출고일자 = '" & DTOS(dtpAppDate.Value) & "') " _
               & "OR (T1.입출고구분 = 1 AND T1.원래입출고일자 = '" & DTOS(dtpAppDate.Value) & "')) "
    ...
    If P_adoRec.RecordCount = 0 Then
       ...
       Exit Sub
    Else
       lngCnt = P_adoRec("입고수량")
       P_adoRec.Close
       If lngCnt = 0 Then
          MsgBox "매입내역이 없어 반품처리할 수 없습니다.", vbCritical, "자재 반품 불가"
          Screen.MousePointer = vbDefault
          Exit Sub
       Else
          If (lngCnt < 1) Or (lngCnt < (Vals(Text1(5).Text) * -1)) Then
             MsgBox "반품수량이 매입수량(" & Format(lngCnt, "#,#") & ") 보다 많아서 반품처리할 수 없습니다.", vbCritical, "자재 반품 불가"
             Text1(5).SetFocus
             Screen.MousePointer = vbDefault
             Exit Sub
          End If
    ...
    With vsfg1
         ...
         P_adoRec.Close
         intRetVal = MsgBox("현금반품으로 처리하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton2, "현금반품")
         ...
         strSQL = "SELECT TOP 1 ISNULL(T1.입고단가, 0) AS 입고단가, ISNULL(T1.입고부가, 0) AS 입고부가 " _
                  & "FROM 자재입출내역 T1 " _
                 & "WHERE (T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "') " _
                   & "AND (T1.분류코드 + T1.세부코드) = '" & Text1(2).Text & "' " _
                   & "AND (T1.매입처코드 = '" & Text1(0).Text & "') " _
                   & "AND (T1.입출고구분 = 1 AND T1.입출고일자 = '" & DTOS(dtpAppDate.Value) & "' AND 입고수량 > 0) "
         On Error GoTo ERROR_TABLE_SELECT
         P_adoRec.Open strSQL, PB_adoCnnSQL, adOpenStatic, adLockReadOnly
         If P_adoRec.RecordCount = 0 Then
            P_adoRec.Close
            ...
            Exit Sub
         Else
            Text1(7).Text = Format(P_adoRec("입고단가"), "#,0.00")
            Text1(8).Text = Format(P_adoRec("입고부가"), "#,0")
         End If
         ...
         If Text1(Text1.LBound).Enabled = True Then '반품 추가
            ...
            strSQL = "INSERT INTO 자재입출내역(사업장코드, 분류코드, 세부코드, " _
                    ...
                    & "1, '" & PB_regUserinfoU.UserClientDate & "', '" & strServerTime & "', " _
            ...
            .AddItem .Rows
            .TextMatrix(.Rows - 1, 0) = Text1(2).Text & "-" & Text1(0) & "-" & "" & "-" & "1" & "-" _
                                      & PB_regUserinfoU.UserClientDate & "-" & strServerTime
            .Cell(flexcpData, .Rows - 1, 0, .Rows - 1, 0) = Trim(.TextMatrix(.Rows - 1, 0))
            .TextMatrix(.Rows - 1, 1) = Format(PB_regUserinfoU.UserClientDate, "0000-00-00") '입출고일자(반품일자)
            .TextMatrix(.Rows - 1, 2) = Format(Mid(strServerTime, 1, 6), "00:00:00")         '입출고시간
            .TextMatrix(.Rows - 1, 3) = Format(DTOS(dtpAppDate.Value), "0000-00-00")         '입고일자
            .TextMatrix(.Rows - 1, 4) = Text1(0).Text: .TextMatrix(.Rows - 1, 5) = Text1(1).Text '매입처
            .TextMatrix(.Rows - 1, 6) = Text1(2).Text: .TextMatrix(.Rows - 1, 7) = Text1(3).Text '자재
            .TextMatrix(.Rows - 1, 8) = Text1(4).Text                                        '규격
            .TextMatrix(.Rows - 1, 9) = Vals(Text1(5).Text)                                  '반품수량
            .TextMatrix(.Rows - 1, 10) = Text1(6).Text                                       '단위
            .TextMatrix(.Rows - 1, 11) = Vals(Text1(7).Text)                                 '입고단가
            .TextMatrix(.Rows - 1, 12) = Vals(Text1(8).Text)                                 '입고부가
            .TextMatrix(.Rows - 1, 13) = .ValueMatrix(.Rows - 1, 9) * .ValueMatrix(.Rows - 1, 11)  '입고금액
            ...
            End If
            .Cell(flexcpText, .Rows - 1, 14) = "현금반품"
         ...
         Else                                          '자재입출내역 변경
            strSQL = "UPDATE 자재입출내역 SET " _
                          ...
                          & "입고수량 = " & Vals(Text1(5).Text) & ", " _
                          & "입고단가 = " & Vals(Text1(7).Text) & ", " _
                          & "입고부가 = " & Vals(Text1(8).Text) & ", " _
                          & "현금구분 = " & intChkCash & ", " _
            ...
            .TextMatrix(.Row, 9) = Vals(Text1(5).Text)                              '반품수량
            .TextMatrix(.Row, 11) = Vals(Text1(7).Text)                             '입고단가
            .TextMatrix(.Row, 12) = Vals(Text1(8).Text)                             '입고부가
            .TextMatrix(.Row, 13) = .ValueMatrix(.Row, 9) * (.ValueMatrix(.Row, 11))  '입고금액
...
ERROR_TABLE_INSERT:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "반품내역 추가 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_UPDATE:
    PB_adoCnnSQL.RollbackTrans
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "반품내역 갱신 실패"
    Unload Me
    Exit Sub
    ...
    With vsfg1
         If .Row >= .FixedRows Then
            ...
            If intRetVal = vbYes Then
               ...
               'P_adoRec.Close
               If lngCnt <> 0 Then
                  MsgBox "XXX(" & Format(lngCnt, "#,#") & "건)가 있으므로 삭제할 수 없습니다.", vbCritical, "반품내역 삭제 불가"
...
ERROR_TABLE_DELETE:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "반품내역 삭제 실패"
    Unload Me
    Screen.MousePointer = vbDefault
    Exit Sub
End Sub
...
Dim lngR    As Long
Dim lngC    As Long
    Text1(Text1.LBound).Enabled = False                '매입처코드 FLASE
    With vsfg1              'Rows 1, Cols 19, RowHeightMax(Min) 300
         ...
         .ColWidth(1) = 1200   '반품일자
         .ColWidth(2) = 1200   '반품시간(시분초)
         .ColWidth(3) = 1200   '입고일자(원래입고일자)
         .ColWidth(4) = 1000   'H, 매입처코드
         .ColWidth(5) = 2000   '매입처명
         .ColWidth(6) = 1900   '자재코드
         .ColWidth(7) = 2500   '자재명
         .ColWidth(8) = 2000   '자재규격
         .ColWidth(9) = 800    '반품수량
         .ColWidth(10) = 600   '자재단위
         .ColWidth(11) = 1500  '입고단가
         .ColWidth(12) = 1300  'H, 입고부가
         .ColWidth(13) = 1600  '입고금액
         .ColWidth(14) = 1200  '매입구분
         .ColWidth(15) = 5000  '적요
         .ColWidth(16) = 1000  '사용자코드
         .ColWidth(17) = 1000  '사용자명
         .ColWidth(18) = 1000  '입출고시간(밀리초까지)
         ...
         .TextMatrix(0, 9) = "수량"
         .TextMatrix(0, 10) = "단위"
         .TextMatrix(0, 11) = "매입단가"
         .TextMatrix(0, 12) = "매입부가"   'H
         .TextMatrix(0, 13) = "매입금액"
         .TextMatrix(0, 14) = "매입구분"
         .TextMatrix(0, 15) = "적요"
         .TextMatrix(0, 16) = "사용자코드" 'H
         .TextMatrix(0, 17) = "사용자명"
         .TextMatrix(0, 18) = "입출고시간"
...
Dim lngRR      As Long
    ...
    strSQL = "SELECT (T1.분류코드 + T1.세부코드) AS 자재코드, ISNULL(T2.자재명,'') AS 자재명, " _
            ...
            & "WHERE T1.사업장코드 = '" & PB_regUserinfoU.UserBranchCode & "' AND T1.사용구분 = 0 " _
              & "AND T1.입출고구분 = 1 AND T1.입고수량 < 0 " _
    ...
    Else
       With vsfg1
            ...
            Do Until P_adoRec.EOF
               ...
               .TextMatrix(lngR, 4) = P_adoRec("매입처코드")
               .TextMatrix(lngR, 5) = P_adoRec("매입처명")
               .TextMatrix(lngR, 6) = P_adoRec("자재코드")
               .TextMatrix(lngR, 7) = P_adoRec("자재명")
               .TextMatrix(lngR, 8) = P_adoRec("자재규격")
               .TextMatrix(lngR, 9) = P_adoRec("입고수량")
               .TextMatrix(lngR, 10) = P_adoRec("자재단위")
               .TextMatrix(lngR, 11) = P_adoRec("입고단가")
               .TextMatrix(lngR, 12) = P_adoRec("입고부가")
...
Dim strForPrtDateTime      As String  '출력일시           (Formula)
    ...
    With CrystalReport1
         If PB_Test = 0 Then
            strExeFile = App.Path & IIf(optPrtChk0.Value = True, ".\반품내역(일자별).rpt", ".\반품내역(업체별).rpt")
         Else
            strExeFile = App.Path & IIf(optPrtChk0.Value = True, ".\반품내역(일자별)T.rpt", ".\반품내역(업체별)T.rpt")
         End If
         ...
         If intRetCHK = 99 Then
            ...
            .WindowTitle = PB_regUserinfoU.UserBranchName & " - " & IIf(optPrtChk0.Value = True, "반품내역(일자별)", "반품내역(업체별)")
            .Action = 1
            .Reset
         End If
    End With
