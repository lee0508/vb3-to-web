...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "발주서 작성(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
               ...
               If (Col = 3) Then         '수량
                  If .TextMatrix(Row, Col) <> .EditText Then
                     ...
                     Else
                        ...
                        .TextMatrix(Row, 11) = Vals(.EditText) * .ValueMatrix(Row, 8)   '수량 * 단가
                     End If
                  End If
               ...
               ElseIf _
                  ...
                  If .TextMatrix(Row, Col) <> .EditText Then                            '변경인 경우 입력금액 검사
                     ...
                     Else
                        ...
                        curTmpMny = .ValueMatrix(Row, 11)
                        .TextMatrix(Row, 10) = Vals(.EditText) + .ValueMatrix(Row, 9)   '입고가격 = 단가 + 부가
                        .TextMatrix(Row, 11) = .ValueMatrix(Row, 3) * Vals(.EditText)   '입고금액 = 수량 * 단가
                     End If
                  End If
               ElseIf _
                  (Col = 9) Then '9.입고부가
                  If .TextMatrix(Row, Col) <> .EditText Then                            '변경인 경우 입력금액 검사
                     ...
                     Else
                        blnModify = True
                        curTmpMny = .ValueMatrix(Row, 11)
                        .TextMatrix(Row, 10) = .ValueMatrix(Row, 8) + Vals(.EditText)   '입고가격 = 단가 + 부가
                        .TextMatrix(Row, 11) = .ValueMatrix(Row, 3) * .ValueMatrix(Row, 8) '입고금액 = 수량 * 단가
                     End If
                  End If
               ElseIf _
    ...
    With vsfg1
         If .Row >= .FixedRows Then     '내역시세검색
            ...
            If KeyCode = vbKeyF2 And (Len(Text1(0).Text) > 0) And (Len(.TextMatrix(.Row, 0)) > 0) Then
               PB_strFMCCallFormName = "frm발주서작성"
...
Dim strMS         As String
    ...
    End With
L_Title:
    strTitle = InputBox("발주서 제목을 입력하세요.", "발주서 제목", "")
    If Not (LenH(strTitle) <= 30) Then '적요
       GoTo L_Title
    End If
    intRetVal = MsgBox("입력된 자료" & lngChkCnt & "(건)을 저장(발주)하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton1, "자료 저장")
    ...
    P_adoRec.CursorLocation = adUseClient
    strSQL = "spLogCounter '발주', '" & PB_regUserinfoU.UserBranchCode + PB_regUserinfoU.UserClientDate & "', " _
    ...
    P_adoRec.Close
    '발주 INSERT
    strSQL = "INSERT INTO 발주(사업장코드, 발주일자, " _
                            & "발주번호, 매입처코드, " _
                            & "입고희망일자, 결제방법, " _
                            & "결제예정일자, 유효일수, " _
                            & "제목, 적요, 상태코드, " _
...
ERROR_TABLE_INSERT:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "발주내역 추가 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_UPDATE:
    PB_adoCnnSQL.RollbackTrans
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "발주내역 저장 실패"
    Unload Me
    Exit Sub
...
Dim lngC    As Long
    With vsfg1              'Rows 101, Cols 17, RowHeightMax(Min) 300
         ...
         .TextMatrix(0, 2) = "규격"
         .TextMatrix(0, 3) = "발주량"
         .TextMatrix(0, 4) = "단위"          '발주단위
         .TextMatrix(0, 5) = "매출처코드"    'H
         .TextMatrix(0, 6) = "매출처명"
         .TextMatrix(0, 7) = "직송"          'H
         .TextMatrix(0, 8) = "발주단가"
         .TextMatrix(0, 9) = "발주부가"
         .TextMatrix(0, 10) = "발주가격"     'H
         .TextMatrix(0, 11) = "발주금액"
         .TextMatrix(0, 12) = "매출단가"     'H
         .TextMatrix(0, 13) = "매출부가"     'H
         .TextMatrix(0, 14) = "매출가격"     'H
            ...
            If optPrtChk1.Value = True Then '발주서
               strSQL = "SELECT SUM(T1.입고단가 * T1.발주량) AS 금액 " _
                       ...
                       & "WHERE T1.사업장코드 = '" & subBranchCode & "' " _
                         & "AND T1.발주일자 = '" & subOrderDate & "' " _
                         & "AND T1.발주번호 = " & subOrderNo & " " _
