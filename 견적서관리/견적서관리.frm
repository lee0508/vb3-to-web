...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적서관리(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
...
Dim lngCnt As Long
    With vsfg1
         If .Row >= .FixedRows Then
            ...
            'vsfg2.Select .ValueMatrix(.Row, 16), vsfg2.FixedCols, .ValueMatrix(.Row, 16), vsfg2.Cols - 1
...
Dim curTmpMny As Currency
    With vsfg2
         If Row >= .FixedRows Then
            If (Col = 16) Then  '수량
               If .TextMatrix(Row, Col) <> .EditText Then
                  'If (IsNumeric(.EditText) = False Or Vals(.EditText) < 0 Or _ '- 수량 입력불가
            ...
            ElseIf _
               ...
               If .TextMatrix(Row, Col) <> .EditText Then
                  If (IsNumeric(.EditText) = False Or Vals(.EditText) < 0 Or _
                     ...
                     'fixVals(.EditText)) < Vals(.EditText) Or IsNumeric(Right(.EditText, 1)) = False) Then '소숫점이하 사용불가
                     Beep
                     Cancel = True
                  Else
                     blnModify = True
...
Dim CtrlDown  As Variant
    With vsfg2
         If (.Row >= .FixedRows) Then     '내역시세검색
            ...
            'If CtrlDown And KeyCode = vbKeyHome And (Len(vsfg1.TextMatrix(vsfg1.Row, 5)) > 0) And
            If KeyCode = vbKeyF2 And (Len(vsfg1.TextMatrix(vsfg1.Row, 5)) > 0) And _
              (Len(.TextMatrix(.Row, 4)) > 0) Then
               PB_strFMCCallFormName = "frm견적서관리"
    ...
    With vsfg2
         ...
         If (.Row >= .FixedRows) Then
            ...
            If KeyCode = vbKeyF1 Then  '자재시세검색
               'If (.MouseCol = 4 Or .MouseCol = 5) Then
                  PB_strFMCCallFormName = "frm견적서관리"
                  PB_strMaterialsCode = .TextMatrix(.Row, 4)
                  PB_strMaterialsName = .TextMatrix(.Row, 5)
                  PB_strSupplierCode = .TextMatrix(.Row, 10)
            ...
            ElseIf _
               ...
               PB_strMaterialsCode = .TextMatrix(.Row, 4)
               PB_strMaterialsName = .TextMatrix(.Row, 5)
               PB_strSupplierCode = .TextMatrix(.Row, 13)        '매입처(Blank)
               frm자재시세검색.Show vbModal
...
Dim strServerTime As String
    If vsfg1.Row >= vsfg1.FixedRows Then
       intRetVal = MsgBox("견적처리된 자료를 저장하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton1, "자료 저장")
       ...
       With vsfg2
            ...
            If vsfg1.Row > 0 Then
               For lngRR = vsfg1.ValueMatrix(vsfg1.Row, 15) To vsfg1.ValueMatrix(vsfg1.Row, 16)
                   If (.TextMatrix(lngRR, 28) = "I") Or (.TextMatrix(lngRR, 28) = "U") Then            '견적내역 변경
...
ERROR_TABLE_INSERT:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적 추가 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_UPDATE:
    PB_adoCnnSQL.RollbackTrans
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적 저장 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_DELETE:
    PB_adoCnnSQL.RollbackTrans
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "견적 삭제 실패"
    Unload Me
    Exit Sub
End Sub
...
Dim lngC    As Long
    With vsfg2              'Rows 1, Cols 30, RowHeightMax(Min) 300
         ...
         .ColWidth(4) = 1900   '자재코드(변경후)
         .ColWidth(5) = 2600   '자재명(변경후)
         ...
         .TextMatrix(0, 23) = "견적단가"
         .TextMatrix(0, 24) = "견적부가"    'H
         .TextMatrix(0, 25) = "견적가격"    'H (단가 + 부가)
         .TextMatrix(0, 26) = "견적금액"
         .TextMatrix(0, 27) = "적요"
         .TextMatrix(0, 28) = "구분"        'H(SQL구분:I.Insert, U.Update, D.Delete)
         .TextMatrix(0, 29) = "견적시간"    'H(견적시간)
    ...
    Else
       With vsfg2
            ...
            Do Until P_adoRec.EOF
               ...
               .TextMatrix(lngR, 8) = IIf(IsNull(P_adoRec("매출처코드")), "", P_adoRec("매출처코드"))  '변경전
               .TextMatrix(lngR, 9) = IIf(IsNull(P_adoRec("매출처명")), "", P_adoRec("매출처명"))
               .TextMatrix(lngR, 10) = IIf(IsNull(P_adoRec("매출처코드")), "", P_adoRec("매출처코드")) '변경후
