Attribute VB_Name = "PubPrtDealTax"
'+-------------------------------------------------------------------------------------------------------+
' 프로그램 제 목 : PubPrtDealTax
' 사용된 Control :
' 참조된 Table   : 자재입출내역, 세금계산서, 매출세금계산서장부, 매출처
' 업  무  설  명 : 거래명세서, 세금계산서 출력
'+-------------------------------------------------------------------------------------------------------+
Option Explicit
...
'/// 거래명세서 분기 ///
'+----------------------+
Public Sub SubPubPrint_DealBill(p As Printer, PB_intPrtTypeGbn As Integer, strDealDate As String, lngLogCnt As Long)
    If PB_intPrtTypeGbn = 1 Then
       SubPubPrint_DealBill_1 p, strDealDate, lngLogCnt
    ElseIf _
       PB_intPrtTypeGbn = 2 Then
       SubPubPrint_DealBill_2 p, strDealDate, lngLogCnt
    ElseIf _
       PB_intPrtTypeGbn = 3 Then
       SubPubPrint_DealBill_3 p, strDealDate, lngLogCnt
    Else
       SubPubPrint_DealBill_1 p, strDealDate, lngLogCnt
    End If
End Sub
...
'/// 세금계산서 분기 ///
'+----------------------+
Public Sub SubPubPrint_TaxBill(p As Printer, PB_intPrtTypeGbn As Integer, PB_strBranchCode As String, strMakeYear As String, lngLogCnt1 As Long, lngLogCnt2 As Long, _
                               intJangGbn As Integer, strMakeDate As String, strMakeTime As String, strSupplierCode As String)
    If PB_intPrtTypeGbn = 1 Then
       SubPubPrint_TaxBill_1 p, PB_strBranchCode, strMakeYear, lngLogCnt1, lngLogCnt2, _
                             intJangGbn, strMakeDate, strMakeTime, strSupplierCode
    ElseIf _
       PB_intPrtTypeGbn = 2 Then
       SubPubPrint_TaxBill_2 p, PB_strBranchCode, strMakeYear, lngLogCnt1, lngLogCnt2, _
                             intJangGbn, strMakeDate, strMakeTime, strSupplierCode
    ElseIf _
       PB_intPrtTypeGbn = 3 Then
       SubPubPrint_TaxBill_3 p, PB_strBranchCode, strMakeYear, lngLogCnt1, lngLogCnt2, _
                             intJangGbn, strMakeDate, strMakeTime, strSupplierCode
    Else
       SubPubPrint_TaxBill_1 p, PB_strBranchCode, strMakeYear, lngLogCnt1, lngLogCnt2, _
                             intJangGbn, strMakeDate, strMakeTime, strSupplierCode
...
'/// 거래명세서 출력(1) ///
'+-------------------------+
Public Sub SubPubPrint_DealBill_1(p As Printer, strDealDate As String, lngLogCnt As Long)
...
Dim lngC              As Long
    ...
    Else
       ...
       C_TMargin = PB_intDTopMargin
       C_LMargin = PB_intDLeftMargin
...
LOOP_A:
    For intA = intForA To UBound(A, 1) - 1
        ...
        If SW_A = 0 Then
           SW_A = 1
           intCurPageCntA = 1
           SubPubPrint_DealBill_HEAD_1 C_TMargin, C_LMargin, intCurPageCntA, intTotPageCnt, strSaleDate, lngSaleNo, _
...
LOOP_B:
    For intB = intForB To UBound(B, 1) - 1
        ...
        If SW_B = 0 Then
           SW_B = 1
           intCurPageCntB = 1
           SubPubPrint_DealBill_HEAD_1 C_TMargin, C_LMargin, intCurPageCntB, intTotPageCnt, strSaleDate, lngSaleNo, _
                                       strBuyerCode, strEnterNo, strEnterName, strRepName, strEnterAddress, curTotMny
        Else
        ...
        If ((intCurPrtCntPerPageB Mod C_intCntPerPage) = 0) Or intPrtCntB = UBound(B, 1) Then
           ...
           Printer.FontSize = 10
           If intPrtCntB = UBound(B, 1) Then
              Printer.Print Space(C_LMargin + 82) & PADL(Format(Vals(A(intB, 6)), "#,0"), 13, "")
           Else
              Printer.Print Space(C_LMargin + 52) & PADL(Space(1), 18, "*") & PADL("(다음장에계속)", 14, "")
           End If
           Printer.NewPage
...
Public Sub SubPubPrint_DealBill_HEAD_1(C_TMargin As Integer, C_LMargin As Integer, _
...
Dim ary0EnterNo12(12)  As String
Dim ary0EnterNo24(24)  As String
Dim str0EnterNo23      As String
...
'/// 세금계산서 출력(1) ///
'+-------------------------+
Private Sub SubPubPrint_TaxBill_1(p As Printer, strBranchCode As String, strMakeYear As String, lngLogCnt1 As Long, lngLogCnt2 As Long, _
                                  intJangGbn As Integer, strMakeDate As String, strMakeTime As String, strSupplierCode As String)
Dim strSQL               As String
'Dim p                    As Printer
Dim strPort              As String
...
Dim lngSeqNo             As Long     '세금계산서 일련번호
    ...
    ElseIf _
       intJangGbn = 1 Then
           strSQL = "SELECT T1.사업장코드 AS 사업장코드, T1.작성년도 AS 작성년도, T1.책번호 AS 책번호, T1.일련번호 AS 일련번호, " _
             ...
             & "FROM 매출세금계산서장부 T1 " _
             & "LEFT JOIN 매출처 T2 ON T2.매출처코드 = T1.매출처코드 " _
            & "WHERE T1.사업장코드 = '" & strBranchCode & "' AND T1.작성일자 = '" & strMakeDate & "' " _
              & "AND T1.작성시간 = '" & strMakeTime & "' AND T1.매출처코드 = '" & strSupplierCode & "' " _
              & "AND T1.사용구분 = 0 " _
    ...
    Else
       ...
       C_TMargin = PB_intTTopMargin
       C_LMargin = PB_intTLeftMargin
    ...
    For intA = LBound(A, 1) To UBound(A, 1) - 1
        '상
        'HEAD
        SubPubPrint_TaxBill_HEAD_1 C_TMargin, C_LMargin, A(intA, 0), A(intA, 1), A(intA, 3), A(intA, 4), A(intA, 5), _
...
Private Sub SubPubPrint_TaxBill_HEAD_1(C_TMargin As Integer, C_LMargin As Integer, _
...
'/// 거래명세서 출력(2) ///
'+-------------------------+
Public Sub SubPubPrint_DealBill_2(p As Printer, strDealDate As String, lngLogCnt As Long)
Dim strSQL               As String
Dim strPort              As String
Dim intFile              As Integer
Dim blnEof               As Boolean
...
LOOP_A:
    For intA = intForA To UBound(A, 1) - 1
        ...
        If SW_A = 0 Then
           SW_A = 1
           intCurPageCntA = 1
           SubPubPrint_DealBill_HEAD_2 C_TMargin, C_LMargin, intCurPageCntA, intTotPageCnt, strSaleDate, lngSaleNo, _
                                       strBuyerCode, strEnterNo, strEnterName, strRepName, strEnterAddress, curTotMny
        Else
           If (intPrtCntA Mod C_intCntPerPage) = 0 Then
              intCurPageCntA = intCurPageCntA + 1
...
LOOP_B:
    For intB = intForB To UBound(B, 1) - 1
        ...
        If SW_B = 0 Then
           SW_B = 1
           intCurPageCntB = 1
           SubPubPrint_DealBill_HEAD_2 C_TMargin, C_LMargin, intCurPageCntB, intTotPageCnt, strSaleDate, lngSaleNo, _
                                       strBuyerCode, strEnterNo, strEnterName, strRepName, strEnterAddress, curTotMny
        Else
           If (intPrtCntB Mod C_intCntPerPage) = 0 Then
              intCurPageCntB = intCurPageCntB + 1
...
Public Sub SubPubPrint_DealBill_HEAD_2(C_TMargin As Integer, C_LMargin As Integer, _
                                       intCurPageCnt As Integer, intTotPageCnt As Integer, _
                                       strSaleDate As String, lngSaleNo As Long, _
                                       strBuyerCode As String, strEnterNo As String, _
                                       strEnterName As String, strRepName As String, _
...
'/// 세금계산서 출력(2) ///
'+-------------------------+
Private Sub SubPubPrint_TaxBill_2(p As Printer, strBranchCode As String, strMakeYear As String, lngLogCnt1 As Long, lngLogCnt2 As Long, _
                                  intJangGbn As Integer, strMakeDate As String, strMakeTime As String, strSupplierCode As String)
Dim strSQL               As String
'Dim p                    As Printer
Dim strPort              As String
...
Dim lngSeqNo             As Long     '세금계산서 일련번호
    ...
    For intA = LBound(A, 1) To UBound(A, 1) - 1
        '상
        'HEAD
        SubPubPrint_TaxBill_HEAD_2 C_TMargin, C_LMargin, A(intA, 0), A(intA, 1), A(intA, 3), A(intA, 4), A(intA, 5), _
                                                         A(intA, 6), A(intA, 7), A(intA, 8), A(intA, 9), A(intA, 15), A(intA, 16)
        'BODY
        '10.월, 11.일, 12.품명및규격, 13.수량, 14.단가, 15.공급가액, 16.세액
        Printer.FontSize = 10
...
Private Sub SubPubPrint_TaxBill_HEAD_2(C_TMargin As Integer, C_LMargin As Integer, _
                                       A0 As String, A1 As String, _
                                       A3 As String, A4 As String, A5 As String, _
                                       A6 As String, A7 As String, A8 As String, _
                                       A9 As String, A15 As String, A16 As String)
...
'/// 거래명세서 출력(3) ///
'+-------------------------+
Public Sub SubPubPrint_DealBill_3(p As Printer, strDealDate As String, lngLogCnt As Long)
Dim strSQL               As String
Dim strPort              As String
Dim intFile              As Integer
Dim blnEof               As Boolean
...
LOOP_A:
    For intA = intForA To UBound(A, 1) - 1
        ...
        If SW_A = 0 Then
           SW_A = 1
           intCurPageCntA = 1
           SubPubPrint_DealBill_HEAD_3 C_TMargin, C_LMargin, intCurPageCntA, intTotPageCnt, strSaleDate, lngSaleNo, _
                                       strBuyerCode, strEnterNo, strEnterName, strRepName, strEnterAddress, curTotMny
        Else
           If (intPrtCntA Mod C_intCntPerPage) = 0 Then
              intCurPageCntA = intCurPageCntA + 1
...
LOOP_B:
    For intB = intForB To UBound(B, 1) - 1
        ...
        If SW_B = 0 Then
           SW_B = 1
           intCurPageCntB = 1
           SubPubPrint_DealBill_HEAD_3 C_TMargin, C_LMargin, intCurPageCntB, intTotPageCnt, strSaleDate, lngSaleNo, _
                                       strBuyerCode, strEnterNo, strEnterName, strRepName, strEnterAddress, curTotMny
        Else
           If (intPrtCntB Mod C_intCntPerPage) = 0 Then
              intCurPageCntB = intCurPageCntB + 1
        ...
        If ((intCurPrtCntPerPageB Mod C_intCntPerPage) = 0) Or intPrtCntB = UBound(B, 1) Then
           ...
           Printer.FontSize = 10
           If intPrtCntB = UBound(B, 1) Then
              Printer.Print Space(C_LMargin + 81) & PADL(Format(Vals(A(intB, 6)), "#,0"), 13, "")
           Else
              Printer.Print Space(C_LMargin + 52) & PADL(Space(1), 18, "*") & PADL("(다음장에계속)", 14, "")
           End If
           Printer.NewPage
...
Public Sub SubPubPrint_DealBill_HEAD_3(C_TMargin As Integer, C_LMargin As Integer, _
                                       intCurPageCnt As Integer, intTotPageCnt As Integer, _
                                       strSaleDate As String, lngSaleNo As Long, _
                                       strBuyerCode As String, strEnterNo As String, _
                                       strEnterName As String, strRepName As String, _
...
Dim inti               As Integer
    ...
    '등록번호 정렬
    For inti = 1 To 12
        ary0EnterNo12(inti) = Mid(PB_strEnterNo, inti, 1)
    Next inti
    For inti = 1 To 12
        If inti = 1 Then
           ary0EnterNo24(inti) = ary0EnterNo12(inti): ary0EnterNo24(inti + 1) = " "
        Else
           ary0EnterNo24(inti * 2 - 1) = ary0EnterNo12(inti): ary0EnterNo24(inti * 2) = " "
        End If
    Next inti
    For inti = 1 To 23
        str0EnterNo23 = str0EnterNo23 + ary0EnterNo24(inti)
    Next inti
    '등록번호 정렬
    For inti = 1 To 12
        aryEnterNo12(inti) = Mid(strEnterNo, inti, 1)
    ...
    '등록번호
    Printer.FontSize = 12
    Printer.Print Space(C_LMargin + 7) & PADR(str0EnterNo23, 23, "") & Space(18) & PADR(strEnterNo23, 23, "")
    'Printer.Print Space(C_LMargin + 50) & Chr(27) & "W1" & PADC(strEnterNo, 14, "") & Chr(27) & "W0"
    Printer.FontSize = 9
    Printer.Print ""
    Printer.FontSize = 2
    ...
    '상호, 성명
    Printer.FontSize = 10
    Printer.Print Space(C_LMargin + 8) & PADR(PB_strEnterName, 20, "") & Space(4) & PADR(PB_strRepName, 16, "") _
                            & Space(8) & PADR(strEnterName, 20, "") & Space(4) & PADR(strRepName, 16, "")
    Printer.Print ""
    '주소(작게)
    Printer.FontSize = 2
    Printer.Print ""
    Printer.Print ""
    Printer.FontSize = 9
    Printer.Print Space(C_LMargin + 10) & PADR(PB_strEnterAddress, 48, "") & Space(4) & PADR(strEnterAddress, 50, "")
    '총금액(크게)
    Printer.FontSize = 8
    Printer.Print "": Printer.Print ""
    Printer.FontSize = 12
...
'/// 세금계산서 출력(3) ///
'+-------------------------+
Private Sub SubPubPrint_TaxBill_3(p As Printer, strBranchCode As String, strMakeYear As String, lngLogCnt1 As Long, lngLogCnt2 As Long, _
                                  intJangGbn As Integer, strMakeDate As String, strMakeTime As String, strSupplierCode As String)
Dim strSQL               As String
'Dim p                    As Printer
Dim strPort              As String
...
Dim lngSeqNo             As Long     '세금계산서 일련번호
    ...
    For intA = LBound(A, 1) To UBound(A, 1) - 1
        '상
        'HEAD
        SubPubPrint_TaxBill_HEAD_3 C_TMargin, C_LMargin, A(intA, 0), A(intA, 1), A(intA, 3), A(intA, 4), A(intA, 5), _
                                                         A(intA, 6), A(intA, 7), A(intA, 8), A(intA, 9), A(intA, 15), A(intA, 16)
        'BODY
        '10.월, 11.일, 12.품명및규격, 13.수량, 14.단가, 15.공급가액, 16.세액
        Printer.FontSize = 10
...
Private Sub SubPubPrint_TaxBill_HEAD_3(C_TMargin As Integer, C_LMargin As Integer, _
                                       A0 As String, A1 As String, _
                                       A3 As String, A4 As String, A5 As String, _
                                       A6 As String, A7 As String, A8 As String, _
                                       A9 As String, A15 As String, A16 As String)
...
Dim strMny2_21        As String
    ...
    Printer.FontSize = 12
    For inti = 1 To 1: Printer.Print "": Next inti
    Printer.Print Space(C_LMargin - 5) & PADR(str0EnterNo23, 23, "") & Space(15) & PADR(strEnterNo23, 23, "")
    For inti = 1 To 1: Printer.Print "": Next inti
    'Printer.Print Space(C_LMargin + 50) & Chr(27) & "W1" & PADC(strEnterNo, 14, "") & Chr(27) & "W0"
    '상호, 성명
    Printer.FontSize = 2: Printer.Print "": Printer.Print "": Printer.Print ""
    Printer.FontSize = 12
    Printer.Print Space(C_LMargin - 8) & PADR(PB_strEnterName, 20, "") & Space(2) & PADR(PB_strRepName, 16, "") _
                                       & Space(0) & PADR(A4, 20, "") & Space(2) & PADR(A5, 16, "")
    '주소(작게)
    Printer.FontSize = 10: Printer.Print "": Printer.Print ""
    Printer.FontSize = 2: Printer.Print "": Printer.Print ""
    Printer.FontSize = 10
    Printer.Print Space(C_LMargin - 7) & PADR(PB_strEnterAddress, 44, "") & Space(2) & PADR(A6, 70, "")
    For inti = 1 To 2: Printer.Print "": Next inti
    '업태, 종목(작게)
    Printer.FontSize = 8
    Printer.Print Space(C_LMargin - 4) & PADR(PB_strUptae, 14, "") & Space(7) & PADR(PB_strUpjong, 14, "") _
                                       & Space(22) & PADR(A7, 14, "") & Space(7) & PADR(A8, 14, "")
    For inti = 1 To 3: Printer.Print "": Next inti
    '작성년월일, 공란수, 공급가액, 세액
    '공급가액 정렬
    For inti = 1 To 11
