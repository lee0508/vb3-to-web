...
Dim strBuffer          As String
    ...
    frmMain.SBar.Panels(4).Text = "세금계산서 수정/삭제시 매출세금계산서장부에는 반영되지 않으며, 미수금 수금내역에도 반영되지 않습니다. "
...
Private Sub vsfg1_MouseUp(Button As Integer, Shift As Integer, x As Single, Y As Single)
    With vsfg1
         ...
         If (.MouseRow >= .FixedRows And _
            .TextMatrix(.MouseRow, 20) = "정상" And .TextMatrix(.MouseRow, 16) = "임의") Then
    ...
    With vsfg1
         If Row >= .FixedRows Then
            ...
            ElseIf _
               (Col = 11) Then  '품목및규격
               If .TextMatrix(Row, Col) <> .EditText Then
                  If .Cell(flexcpChecked, Row, 19) = flexChecked Then '미수면(거래내역 있으면)
    ...
    If optPrtChk0.Value = True Then '세금계산서인쇄(건별)
       ...
       With vsfg1
            If .TextMatrix(.Row, 20) = "정상" Then
               ...
               End If
               SubPubPrint_TaxBill p, PB_intPrtTypeGbn, .TextMatrix(.Row, 0), .TextMatrix(.Row, 1), .ValueMatrix(.Row, 2), .ValueMatrix(.Row, 3), _
                                   0, "", "", ""
            End If
       End With
    End If
       ...
       With vsfg1
            For lngR = 1 To .Rows - 1
                If .TextMatrix(lngR, 20) = "정상" Then
                   ...
                   End If
                   SubPubPrint_TaxBill p, PB_intPrtTypeGbn, .TextMatrix(lngR, 0), .TextMatrix(lngR, 1), .ValueMatrix(lngR, 2), .ValueMatrix(lngR, 3), _
                                       0, "", "", ""
               End If
            Next lngR
       End With
...
Dim strJukyo       As String  '자재입출내역의 적요
    ...
    If vsfg1.Row >= vsfg1.FixedRows Then
       With vsfg1
            ...
            End If
       End With
       intRetVal = MsgBox("수정된 세금계산서를 저장하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton1, "자료 저장")
       ...
       P_adoRec.CursorLocation = adUseClient
       With vsfg1
            If .Cell(flexcpChecked, .Row, 19) = flexChecked Then '미수금(거래입출내역) 처리면
               If .ValueMatrix(.Row, 13) = 0 Then
                  strJukyo = .TextMatrix(.Row, 11)
               Else
                  strJukyo = .TextMatrix(.Row, 11) + " 외 " + .TextMatrix(.Row, 13) + "종"
               End If
            End If
            If .Cell(flexcpChecked, .Row, 19) = flexChecked Then '미수금(거래입출내역) 처리면
       ...
       If (chkPrint.Value = 1) And (cboPrinter.ListIndex >= 0) Then                        '세금계산서 출력
          With vsfg1
               SubPubPrint_TaxBill p, PB_intPrtTypeGbn, .TextMatrix(.Row, 0), .TextMatrix(.Row, 1), .TextMatrix(.Value, 2), .ValueMatrix(.Row, 3), _
                                   0, "", "", ""
          End With
       End If
       Screen.MousePointer = vbDefault
...
Dim lngLogCnt  As Long
    If (vsfg1.Row >= vsfg1.FixedRows And vsfg1.TextMatrix(vsfg1.Row, 20) = "정상") Then
       intRetVal = MsgBox("세금계산서를 삭제하시겠습니까 ?", vbCritical + vbYesNo + vbDefaultButton2, "세금계산서 삭제")
       If intRetVal = vbNo Then
          vsfg1.SetFocus
          Exit Sub
       End If
       'intRetVal = MsgBox("세금계산서만(거래내역[미수금발생내역]제외)을 삭제하시겠습니까 ? " + vbCr _
       '         & vbCr + "[예(Y)] 세금계산서만 삭제, [아니오(N)] 세금계산서및 전표내역삭제, [취소] 삭제 취소", "" _
       '         & vbCritical + vbYesNoCancel + vbDefaultButton1, "세금계산서 삭제")
       'If intRetVal = vbCancel Then
       '   vsfg1.SetFocus
       '   Exit Sub
       'End If
       'If intRetVal = vbNo Then
       '   intRetVal = MsgBox("거래내역[미수금발생내역] 전체 삭제를 선택하셨습니다. 다시한번 확인해주세요 ! " + vbCr _
       '         & vbCr + "[예(Y)] 세금계산서만 삭제, [아니오(N)], 세금계산서및 전표내역삭제, [취소] 삭제 취소", "" _
       '         & vbCritical + vbYesNoCancel + vbDefaultButton2, "세금계산서 삭제")
       '   If intRetVal = vbCancel Then
       '      vsfg1.SetFocus
       '      Exit Sub
       '   End If
       ...
       With vsfg1
            ...
            PB_adoCnnSQL.BeginTrans
            If intRetVal = vbYes Or intRetVal = vbNo Then '세금계산서 삭제
               strSQL = "UPDATE 세금계산서 SET " _
                             & "사용구분 = 9, " _
                             & "수정일자 = '" & PB_regUserinfoU.UserServerDate & "', " _
                             & "사용자코드 = '" & PB_regUserinfoU.UserCode & "' " _
               ...
               PB_adoCnnSQL.Execute strSQL
            End If
            'If intRetVal = vbNo Then  '거래내역 식제
            '   strSQL = "UPDATE 자재입출내역 SET " _
            '                 & "사용구분 = 9, " _
            '                 & "수정일자 = '" & PB_regUserinfoU.UserServerDate & "', " _
            '                 & "사용자코드 = '" & PB_regUserinfoU.UserCode & "' " _
            ...
            '   PB_adoCnnSQL.Execute strSQL
            'ElseIf _
               intRetVal = vbYes Then  '세금계산서 번호 제거
               strSQL = "UPDATE 자재입출내역 SET " _
                             & "계산서발행여부 = 0, " _
                             & "작성년도 = '', 책번호 = 0, 일련번호 = 0, " _
                             & "수정일자 = '" & PB_regUserinfoU.UserServerDate & "', " _
...
Dim lngC    As Long
    With vsfg1              'Rows 1, Cols 22, RowHeightMax(Min) 300
         ...
         .ColWidth(4) = 1450   '세금계산서번호(작성년도-책번호-일련번호)
         .ColWidth(5) = 1000   '발행일자(작성일자)
...
Dim lngRR      As Long
    ...
    If optDate.Value = True Then '발행일자
       ...
       strOrderBy = "ORDER BY T1.사업장코드, T1.작성일자, T1.작성년도, T1.책번호, T1.일련번호 "
    End If
