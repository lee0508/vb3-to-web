...
ERROR_FORM_ACTIVATE_CONNECTION_SERVER:
    MsgBox Err.Number & Err.Description & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "발주서관리(서버와의 연결 실패)"
    Unload Me
    Exit Sub
End Sub
            ...
            If (Col = 16) Then  '발주량
               If .TextMatrix(Row, Col) <> .EditText Then
                  ...
                  Else
                     ...
                     .TextMatrix(Row, 22) = Vals(.EditText) * .ValueMatrix(Row, 19)  '합계금액 = 발주량 * 입고단가
                  End If
               End If
            ElseIf _
               (Col = 18) Then  '직송구분
            ...
            ElseIf _
               ...
               If .TextMatrix(Row, Col) <> .EditText Then
                  ...
                  Else
                     ...
                     .TextMatrix(Row, 22) = .ValueMatrix(Row, 16) * Vals(.EditText)                          '합계금액 = 수량 * 입고단가
                  End If
               End If
            ElseIf _
               (Col = 20) Then  '입고부가
         ...
         If (.Row >= .FixedRows) Then     '내역시세검색
            ...
            If KeyCode = vbKeyF2 And (Len(vsfg1.TextMatrix(vsfg1.Row, 5)) > 0) And _
              (Len(.TextMatrix(.Row, 4)) > 0) Then
               PB_strFMCCallFormName = "frm발주서관리"
    ...
    With vsfg2
         ...
         If (.Row >= .FixedRows) Then
            ...
            ElseIf _
               ...
               End If
            ElseIf _
               KeyCode = vbKeyDelete And (.Col <> 9) And (.MouseRow > 0) Then
...
Dim lngLogCnt  As Long
    If vsfg1.Row >= vsfg1.FixedRows Then
       intRetVal = MsgBox("발주처리된 자료를 저장하시겠습니까 ?", vbQuestion + vbYesNo + vbDefaultButton1, "자료 저장")
       ...
       With vsfg2
            ...
            For lngRR = vsfg1.ValueMatrix(vsfg1.Row, 15) To vsfg1.ValueMatrix(vsfg1.Row, 16)
                If (.TextMatrix(lngRR, 28) = "I") Then '발주내역 추가
                ...
                ElseIf _
                   ...
                   PB_adoCnnSQL.Execute strSQL
                ElseIf _
                   (.TextMatrix(lngRR, 28) = "U") Then '발주내역 변경
       ...
       With vsfg2
            ...
            If vsfg1.Row > 0 Then
               For lngRR = vsfg1.ValueMatrix(vsfg1.Row, 15) To vsfg1.ValueMatrix(vsfg1.Row, 16)
                   If (.TextMatrix(lngRR, 28) = "I") Or (.TextMatrix(lngRR, 28) = "U") Then    '발주내역 변경
...
ERROR_TABLE_INSERT:
    ...
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "발주 추가 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_UPDATE:
    PB_adoCnnSQL.RollbackTrans
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "발주 저장 실패"
    Unload Me
    Exit Sub
ERROR_TABLE_DELETE:
    PB_adoCnnSQL.RollbackTrans
    MsgBox Err.Number & Err.Description & _
           vbCr & strSQL & _
           vbCr & "서버와의 연결에 실패했습니다. 프로그램을 종료합니다.", vbCritical, "발주 삭제 실패"
    Unload Me
    Exit Sub
End Sub
            ...
            For lngRR = .ValueMatrix(.Row, 16) To .ValueMatrix(.Row, 15) Step -1
                strSQL = "UPDATE 발주내역 SET " _
                        ...
                        & "WHERE 사업장코드 = '" & vsfg2.TextMatrix(lngRR, 1) & "' " _
                          ...
                          & "AND 발주시간 = '" & vsfg2.TextMatrix(lngRR, 29) & "' " _
...
Dim lngC    As Long
    With vsfg2              'Rows 1, Cols 30, RowHeightMax(Min) 300
         ...
         .TextMatrix(0, 9) = "매출처명"     'H, 변경후 (Not Used)
         .TextMatrix(0, 10) = "매출처코드"  'H, 변경전
         .TextMatrix(0, 11) = "매출처명"    'H, 변경전
         .TextMatrix(0, 12) = "KEY"         'H, 변경전
         ...
         .TextMatrix(0, 17) = "단위"
         .TextMatrix(0, 18) = "직송"
         .TextMatrix(0, 19) = "발주단가"
         .TextMatrix(0, 20) = "발주부가"    'H
         .TextMatrix(0, 21) = "발주가격"    'H (단가 + 부가)
         .TextMatrix(0, 22) = "발주금액"
    ...
    Else
       With vsfg2
            ...
            Do Until P_adoRec.EOF
               ...
               .TextMatrix(lngR, 22) = .ValueMatrix(lngR, 16) * .ValueMatrix(lngR, 19)  '합계금액 = 수량 * 단가
               ...
               .TextMatrix(lngR, 26) = .ValueMatrix(lngR, 16) * .ValueMatrix(lngR, 23)  '합계금액 = 수량 * 단가
